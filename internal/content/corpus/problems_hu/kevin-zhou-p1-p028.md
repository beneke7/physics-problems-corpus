...
language: "hu"
translated: true
links_en: "kevin-zhou-p1-p028"
topic: [Matematikai módszerek]
subtopic: [dimenzióanalízis, geometria, differenciálok]
math_tools: [dimenzióanalízis, differenciálszámítás]
format: "open-ended"
kind: "problem"
core_ideas: []
has_solution: false
...
[2] Feladat 28. Consider a triangle with side lengths a, b, and c. It turns out the area of its incircle
...
finite quantities, and treating derivatives like fractions is “illegal”. But it’s also very useful.
    ...
    for a function f (x(t), y(t)), the chain rule says
                                        ...
                                        df   ∂f dx ∂f dy
                                           =       +
                                        dt   ∂x dt   ∂y dt
    ahol there are two terms, representing the change in f from changes only in x, and only
    in y. Therefore, when we start studying thermodynamics, ahol multivariable functions are
    ...
    number 2, we would write it as S(1) ahol S is the successor function, obeying properties
