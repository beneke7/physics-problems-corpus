---
id: eotvos-2002-gymnast-rings
difficulty_level: 3
source: eotvos
native_id: "2002 P1"
year: 2002
language: hu
translated: false
links_en: eotvos-2002-gymnast-rings
topic: [mechanics]
subtopic: [rotational-dynamics, force-balance, energy-conservation, dynamics]
format: open-ended
math_tools: [calculus, differential-equations, geometry-trigonometry]
core_ideas: [kalda-mechanics-idea35]
has_figure: false
figure_files: []
provenance_note: "Eötvös Competition 2002, Problem 1. Mathpix OCR of born-digital PDF (cache/phoxiv/eotvos/2002.pdf). Author: Takács Gábor. Figures present in source but not localized."
verification_status: pending
has_solution: true
solution_language: hu
techniques: [conservation-laws, smart-coordinates]
---

**1. Tornász at gyűrűn**

Szeretnénk megbecsülni, hogy mekkora erő feszíti a gyűrűn edzó tornász karjait pályájának legalsó pontján, ha kézenállásból óriáskörbe lendül. A következő egyszerű modellt alkalmazzuk: igen hosszú, nyújthatatlan kötél végére $l$ hosszúságú, $m$ tömegű homogén rudat erősítünk, és az 1.a ábrán látható helyzetből elengedjük. Számítsuk ki, hogy mekkora erő feszíti a kötelet az 1.b ábrán látható pillanatban! (A kötél tömegét hanyagoljuk el!)

(Takács Gábor feladata nyomán)
