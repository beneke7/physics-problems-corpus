...
id: inpho-2022-a1
source: inpho
native_id: "1. A block of mass $m=0.1 \mathrm{~kg}$ is attached to a spring (one end fixed to the wall) with spring constant $k=50 \mathrm{~N} \mathrm{~m}^{-1}$. The block slides on a rough horizontal table along the $x$-axis. Assume that both the coefficients of kinetic $\left(\mu_{k}\right)$ and static friction $\left(\mu_{s}\right)$ are same and constant $\left(\mu_{k}=\mu_{s}=\mu=0.25\right)$. The block is initially displaced to $x_{0}=0.1 \mathrm{~m}$ from the unstretched position (normal length of the spring, $x=0$ ) of the spring and released from rest as shown below. Neglect any air resistance. Take the acceleration $g$ due to gravity to be $10 \mathrm{~m} / \mathrm{s}^{2}$."
year: 2022
language: en
translated: false
topic: [mechanics]
subtopic: [oscillations-mechanical, friction, elasticity]
difficulty: null
difficulty_norm: 4
difficulty_level: 4
math_tools: []
...
has_solution: true
has_figure: true
figure_files: [inpho-2022-a1-fig01.png]
provenance_note: "phoXiv Mathpix Markdown: cache/phoxiv/inpho/2022.mmd; figures require local harvest"
