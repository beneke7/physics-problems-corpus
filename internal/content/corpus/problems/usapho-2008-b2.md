...
has_solution: true
has_figure: true
figure_files: [usapho-2008-b2-fig01.png, usapho-2008-b2-fig02.png, usapho-2008-b2-fig03.png]
provenance_note: "phoXiv Mathpix Markdown: cache/phoxiv/usapho/2008.mmd; figures require local harvest"
...
b. Suppose instead that the ball of mass $M$ and charge $q$ is released from rest at a point halfway between the plates at a time $t=0$. Now, an AC potential difference $V(t)=V_{0} \sin \omega t$ is also placed across the capacitor. The ball may hit one of the plates before it falls (under the influence of gravity) out of the region between the plates. If $V_{0}$ is sufficiently large, this will only occur for some range of angular frequencies $\omega_{\text {min }}<\omega<\omega_{\text {max }}$. You may assume that $\omega_{\text {min }} \ll \sqrt{g / h}$ and $\omega_{\text {max }} \gg \sqrt{g / h}$. Making these assumptions, find expressions for $\omega_{\text {min }}$ and $\omega_{\text {max }}$ in terms of the given quantities and/or fundamental constants.
...
$$
\frac{1}{f}=\left(n_{\text {liquid }}-n_{\text {air }}\right)\left(\frac{1}{R_{1}}-\frac{1}{R_{2}}\right),
$$
where $n_{\text {liquid }}$ and $n_{\text {air }}$ are the refractive indices of the liquid in the lens and air around it, and $R_{1}$ and $R_{2}$ are the radii of curvature of the two surfaces of the lens. In figure 1, $R_{1}$ is the curved face and $R_{2}$ is the flat face. Use the lensmaker's formula to calculate the focal length of the lens in terms of the total volume of the liquid, the contact angle, and the relevant refractive indices.
