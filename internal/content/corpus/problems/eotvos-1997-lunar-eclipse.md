---
id: eotvos-1997-lunar-eclipse
difficulty_level: 4
source: eotvos
native_id: "1997 P1"
year: 1997
language: hu
translated: false
links_en: eotvos-1997-lunar-eclipse
topic: [astronomy, optics, mechanics]
subtopic: [orbital-mechanics, shadow-geometry, atmospheric-refraction, angular-size]
format: open-ended
math_tools: [geometry, algebra, trigonometry]
core_ideas: []
has_figure: false
figure_files: []
provenance_note: "Eötvös Competition 1997, Problem 1, by Gyula Radnai. Mathpix OCR of the born-digital source PDF (cache/phoxiv/eotvos/1997_S.pdf) which contains both problems and solutions. OCR corrections: 'Negyed 10-tốl' → 'Negyed 10-tól' (accent diacritic normalization)."
verification_status: pending
has_solution: true
solution_language: hu
techniques: [smart-coordinates, scaling-and-dimensional-analysis]
---

Magyarországon 1997. szeptember 16-án este holdfogyatkozást lehetett megfigyelni. Negyed 10-tól negyed 11-ig tartott a teljes holdfogyatkozás, vagyis ekkor tartózkodott a Hold teljes egészében a Föld teljes árnyékában.

a) A holdfogyatkozás általában hosszabb ideig tart, mint a napfogyatkozás. Miért?

b) Feltételezve, hogy a Hold a Föld körül és a Föld a Nap körül körpályán kering, valamint elhanyagolva a Föld légkörének optikai hatását, határozzuk meg, hogy legfeljebb mennyi ideig tarthat egy teljes holdfogyatkozás! A Földről a Hold és a Nap egyaránt 0,5° látószögben látszik; a Föld látószöge a Holdról nézve 1,83°. (Hogyan befolyásolná az eredményt, ha figyelembe vennénk, hogy az említett pályák inkább ellipszisek, mint körök?)

c) A valóságban teljes holdfogyatkozáskor sem tünik el teljesen a Hold az égról, hanem vöröses színben, halványan világít. Miért?
