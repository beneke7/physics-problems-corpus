---
id: eotvos-2009-cylindrical-mirror
difficulty_level: 4
source: eotvos
native_id: "2009 P2"
year: 2009
language: hu
translated: false
links_en: eotvos-2009-cylindrical-mirror
topic: [optics]
subtopic: [geometrical-optics, reflection, cylindrical-mirrors, image-formation]
format: open-ended
math_tools: [geometry-trigonometry, algebra]
core_ideas: [kalda-waveoptics-idea2]
has_figure: false
figure_files: []
provenance_note: "Eötvös Competition 2009, Problem 2 (author: Radnai Gyula). Mathpix OCR of the Hungarian problem statement in cache/phoxiv/eotvos/2009_S.pdf (combined solutions PDF containing both problems and solutions in Hungarian); statement-only extraction."
verification_status: pending
has_solution: true
solution_language: hu
techniques: [model-reduction, limiting-and-special-cases]
---

**2. feladat. Láng képe hengertükörben.**

Kör alakú asztal közepén áll egy nagyon vékony falú, hengeres üvegváza, amelyben egy gyertya ég. A henger átmérője 12 cm, tengelye függőleges, a láng közepe 2 cm-re van a váza tengelyétől.

Laci oldalról, a lánggal azonos magasságból nézi a vázát, és felfigyel arra, hogy a láng mellett a lángnak egy éles, határozott tükörképe is látszik a váza belsejében. Az asztalt körbejárva megállapítja, hogy a láng képének a szélessége és a vázához viszonyított helye folyamatosan változik.

a) Milyen irányból nézve látszik a láng képe ugyanolyan szélesnek, mint maga a láng?

b) Milyen pályán mozog a láng képének a közepe, miközben Laci körbejárja az asztalt?

A hengertükör leképezésére alkalmazhatjuk a gömbtükörre érvényes leképezési törvényt.

*(Radnai Gyula)*
