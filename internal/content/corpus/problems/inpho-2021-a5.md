...
id: inpho-2021-a5
source: inpho
native_id: "5. Two friends, Amina (A) and Beena (B), are sitting at diametrically opposite points of a merrygo-round (taken as a circular disk in the horizontal plane) of radius $R$ that is rotating at constant angular speed $\omega$ in the anticlockwise direction, when viewed from the top (see figure below)."
year: 2021
language: en
translated: false
topic: [mechanics]
subtopic: [circular-motion, non-inertial-frames, centrifugal, projectile-motion]
difficulty: null
difficulty_norm: 5
difficulty_level: 5
...
has_solution: false
has_figure: true
figure_files: [inpho-2021-a5-fig01.png]
provenance_note: "phoXiv Mathpix Markdown: cache/phoxiv/inpho/2021.mmd; figures require local harvest"
...
![](../../figures/inpho/figures/inpho-2021-a5-fig01.png)
    (a) [6 marks] Determine $u, \theta$ and $\phi$, in terms of $R, \omega, \alpha$, and other relevant quantities.
    (b) [ $\mathbf{3}$ marks] If Amina throws the ball with $\phi=60^{\circ}$, and appropriate values of $\theta$ and $u$ such that Beena can catch it, what is the magnitude of the displacement, $s$, of the ball when it is caught by Beena? For this part only, take $R=1.5 \mathrm{~m}$, and it is enough to state your answer within a range of 0.5 m.
    (c) [0.5 marks] Determine the speed of throwing $u_{\mathrm{D}}$ if Beena catches the ball at the point D $\left(\angle B O D=90^{\circ}\right)$, instead of C.
    (d) [3 marks] What should be the angular speed $\omega_{m}$ of the merry-go-round for which the speed of throwing $u_{\mathrm{D}}$ will be minimum for Beena to catch the ball at the position D? What is this minimum speed of throwing $u_{m}$ ?
