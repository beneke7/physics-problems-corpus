...
id: inpho-2011-a2
source: inpho
native_id: "2. In a modified Young's double slit experiment the region between screen and slits is immersed in a liquid whose refractive index varies with time $t$ (in seconds) as $\mu_{l}=$ $2.50-0.25 t$ until it reaches a steady state value 1.25. The distance between the slits and the screen is $D=1.00 \mathrm{~m}$ and the distance between the slits $S_{1}$ and $S_{2}$ is $d=2.00 \times 10^{-3} \mathrm{~m}$. A glass plate of thickness $t_{g}=3.60 \times 10^{-5} \mathrm{~m}$ and refractive index $\mu_{g}=1.50$ is introduced in front of one of the slits. Note that the illuminations at $S_{1}$ and $S_{2}$ are from coherent sources with zero phase difference."
year: 2011
language: en
translated: false
...
has_solution: false
has_figure: true
figure_files: [inpho-2011-a2-fig01.png]
