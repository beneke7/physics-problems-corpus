...
id: inpho-2008-a7
source: inpho
native_id: "7. An $N$ turn metallic ring of radius $a$, resistance $R$, and inductance $L$ is held fixed with its axis along a spatially uniform magnetic field $\vec{B}$ whose magnitude is given by $B_{0} \sin (\omega t)$."
year: 2008
language: en
translated: false
...
7. An $N$ turn metallic ring of radius $a$, resistance $R$, and inductance $L$ is held fixed with its axis along a spatially uniform magnetic field $\vec{B}$ whose magnitude is given by $B_{0} \sin (\omega t)$.
...
(d) Calculate the time-averaged compressional force per unit length given that $B_{0}=1$ tesla, $N=10, a=10.0 \mathrm{~cm}, \omega=1000.0$ radians/sec, $R=10 \Omega, L=100 \mathrm{mH}$.
(e) Answer the following two questions without providing rigorous justification:
    i. For $\omega / 2 \pi=60 \mathrm{~Hz}$, the ring emits a humming sound. What is the frequency of this sound.
