---
id: inpho-2019-a7
source: inpho
native_id: "7. As shown, a magnet of mass $m=19.00 \mathrm{~g}$ slides on a rough non-magnetic metallic inclined plane which makes an angle $\theta$ with the horizontal. One can change the angle of inclination of the plane. Due to relative motion between the magnet and the plane, eddy currents are generated inside the metal which retard motion of the magnet."
year: 2019
language: en
translated: false
topic: [mechanics, electromagnetism]
subtopic: [friction, force-balance, induction, measurement]
difficulty: null
difficulty_norm: 4
math_tools: []
format: experimental
core_ideas: []
estimated_time_min: 60
points: null
has_solution: true
has_figure: true
figure_files: [inpho-2019-a7-fig01.png, inpho-2019-a7-fig02.png, inpho-2019-a7-fig03.png, inpho-2019-a7-fig04.png, inpho-2019-a7-fig05.png, inpho-2019-a7-fig06.png, inpho-2019-a7-fig07.png, inpho-2019-a7-fig08.png, inpho-2019-a7-fig09.png, inpho-2019-a7-fig10.png]
provenance_note: "phoXiv Mathpix Markdown: cache/phoxiv/inpho/2019.mmd; figures require local harvest"
verification_status: pending
solution_language: en
techniques: [graphical-reduction, differential-element]
---

7. As shown, a magnet of mass $m=19.00 \mathrm{~g}$ slides on a rough non-magnetic metallic inclined plane which makes an angle $\theta$ with the horizontal. One can change the angle of inclination of the plane. Due to relative motion between the magnet and the plane, eddy currents are generated inside the metal which retard motion of the magnet.
Assume that the magnitude of the resistive force on the magnet is $b v$ where $b$ is a positive constant and $v$ is the instantaneous speed of the magnet. Let $\mu$ be the coefficient of kinetic friction between the magnet and the plane.
    (a) If the magnet starts moving from rest at time $t=0$, obtain the expression of its terminal velocity $V_{\mathrm{T}}$. Also obtain the displacement $S(t)$ along the inclined plane as a function of

time. Take $S(0)=0$.
$$
V_{\mathrm{T}}=
$$
$$
S(t)=
$$
(b)For a fixed $\theta$ a student records $S$ for all values of $t$ as shown in the table below. Draw a suitable graph and obtain the terminal velocity $V_{\mathrm{T}}$ of the magnet from this graph. For this and the next part, three graph papers are provided with this booklet. No extra graph papers will be provided.

| $t(\mathrm{~s})$ | $S(\mathrm{~m})$ |
| :--- | :--- |
| 0.016 | 0.001 |
| 0.049 | 0.003 |
| 0.070 | 0.006 |
| 0.090 | 0.010 |
| 0.120 | 0.017 |
| 0.174 | 0.029 |
| 0.230 | 0.046 |
| 0.270 | 0.058 |
| 0.320 | 0.074 |
| 0.370 | 0.091 |

$$
V_{\mathrm{T}}=
$$

Graph is plotted on page no. :
(c)The above process is repeated for various values of $\theta$. The obtained values of terminal velocities for the different $\theta$ are given below.

| $\theta$ (degree) | $V_{\mathrm{T}}(\mathrm{m} / \mathrm{s})$ |
| :--- | :--- |
| 19 | 0.15 |
| 24 | 0.23 |
| 28 | 0.29 |
| 35 | 0.40 |
| 40 | 0.46 |
| 45 | 0.53 |
| 48 | 0.58 |
| 52 | 0.62 |

Plot a suitable graph and obtain the values of $b$ and $\mu$ from this graph.

| $b=$ | $\mu=$ |
| :--- | :--- |

Graph is plotted on page no. : $\_\_\_\_$

Detailed answers can be found on page numbers:
□
![](inpho-2019-a7-fig01.png)
□
□
□
![](inpho-2019-a7-fig02.png)
□
□
□
![](inpho-2019-a7-fig03.png)
□
□
□
![](inpho-2019-a7-fig04.png)
□
□
□
![](inpho-2019-a7-fig05.png)
□
□
□
![](inpho-2019-a7-fig06.png)
□
□
□
![](inpho-2019-a7-fig07.png)
□
□
□
![](inpho-2019-a7-fig08.png)
□
□
□
![](inpho-2019-a7-fig09.png)
□ Last four digits of Roll No.:

|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |

□

|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |

$\_\_\_\_$

|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |

□
![](inpho-2019-a7-fig10.png)
