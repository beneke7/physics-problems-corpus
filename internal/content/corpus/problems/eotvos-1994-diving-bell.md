---
id: eotvos-1994-diving-bell
source: eotvos
native_id: "1994 P1"
year: 1994
language: hu
translated: false
links_en: eotvos-1994-diving-bell
topic: [thermodynamics, mechanics, fluids]
subtopic: [isothermal-process, ideal-gas, pressure, hydrostatics, work]
format: open-ended
math_tools: [calculus-integral, algebra]
core_ideas: [kalda-thermo-idea10]
has_figure: false
figure_files: []
provenance_note: "Eötvös Competition 1994, Problem 1, by Frigyes Károlyházy. Mathpix OCR of the born-digital source PDF (cache/phoxiv/eotvos/1994_S.pdf) which contains both problems and solutions."
verification_status: pending
has_solution: true
solution_language: hu
techniques: [conservation-laws, differential-element]
---

**1. feladat. Búvárharang.**

Egy tóba 20 m mélyre lesúllyesztett, $1 \mathrm{~m}^{3}$ ürtartalmú búvárharang megtelt vízzel. A felszínen úszó hajóból vékony csövön át levegőt pumpálunk a harang alá. (A harang súlyos, még ekkor sem emelkedik fel.) A levegő és a víz hőmérséklete között nincs számottevő különbség.

| Legalább mekkora munkát végez a kompresszor az $1 \mathrm{~m}^{3}$ víz kiszorítása során?

*Károlyházy Frigyes*
