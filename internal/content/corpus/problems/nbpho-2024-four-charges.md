---
id: nbpho-2024-four-charges
source: nbpho
native_id: "2024 P1 Four charges"
year: 2024
language: en
translated: false
links_hu: nbpho-2024-four-charges
topic: [electromagnetism, mechanics]
subtopic: [coulomb-force, charge-dynamics, motion]
math_tools: [vector-calculus, geometry]
format: open-ended
points: 7
core_ideas: []
has_figure: true
figure_files: []
provenance_note: "NBPhO 2024 (Nordic-Baltic Physics Olympiad), Problem 1, by Päivo Simson and Jaan Kalda. Mathpix OCR of cache/phoxiv/nbpho/2024.pdf. Statement figure extraction from PDF pending."
verification_status: pending
has_solution: true
solution_language: en
techniques: [exploit-symmetry, conservation-laws]
---

**1. Four charges (7 points)** — *Päivo Simson, Jaan Kalda.*

Four identical particles are initially in the corners of a square, as shown in the figure below. All particles have the same charge $q$, mass $m$, and the same magnitude of initial velocity $v_{0}$. The directions of the initial velocities are indicated in the figure. You can assume $v \ll c$ and ignore gravity.

**i)** *(2 points)* After a long time has passed, what is the magnitude of the final velocity $v_{f}$ of the particles with respect to the center of mass of the system?

**ii)** *(5 points)* What is the angle between the initial velocity $\vec{v}_{0}$ and the final velocity $\vec{v}_{f}$ of a particle?
