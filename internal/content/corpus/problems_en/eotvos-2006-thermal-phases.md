---
id: eotvos-2006-thermal-phases
source: eotvos
native_id: "2006 P1"
year: 2006
language: en
translated: true
links_hu: eotvos-2006-thermal-phases
topic: [thermodynamics]
subtopic: [heat-transfer, phase-transition, thermal-equilibrium, thermal-conductivity]
format: open-ended
math_tools: [calculus, algebra]
core_ideas: []
has_figure: true
figure_files: []
provenance_note: "English translation of Eötvös Competition 2006, Problem 1. Translated from Hungarian source statement (cache/phoxiv/eotvos/2006_S.pdf)."
verification_status: pending
---

**Problem 1. Phases in Heat Conduction**

In physics club, the following problem comes up from a problem book:

"A vertical cylindrical container is filled with water to about half its height and then sealed. The top and bottom plates are good heat conductors, while the cylinder's side wall is thermally insulating. The bottom plate is cooled to −10 °C, the top plate is heated to 110 °C, and we maintain it at these temperatures. After a long time, how are the different states of matter distributed along the height in the container?"

The students research in various books. Tóni thinks that ice floats on water, so the liquid water must be at the bottom. Réka thinks water must be in the middle because it contacts hot vapor. Bea, while searching for data, discovers that in some tables—apparently due to a typo—the thermal conductivity of gases is listed as greater than that of water or ice. However, according to other tables and books, the thermal conductivity of gases is many times smaller. (Bea agrees that this makes more sense.)

**Help them find the correct answer to the question in the problem!**

