---
id: solution-document-rupho-w-2023-pe-s
source: rupho-w
...
mapped_problems: [rupho-w-2023-pe]
verification_status: pending
provenance_note: "Source-provided solution document; preserved from cache/phoxiv/rupho-w/2023/PE_S.pdf."
---
...
A1 0.41   Для каждого x найдите n.
 ...
 A2 0.82   Для каждого x найдите λ.
 ...
 A3 0.82   Для каждого x найдите J (в условных единицах на нм). Поскольку для каждого x мы уже вычислили λ, мы получили зависимость J (λ).
                                     λ                                                                                               λ
 ...
 A6 1.23   Найдите для каждого x значение σ. Поскольку для каждого x мы нашли λ, мы получили также зависимость λ(σ).


Для каждого x найдём значение суммы
               i
...
Поскольку λ(x) -- убывающая функция, то σ = 0 соответствует значение x = x   max     , поэтому σ рассчитывается по формуле:
                                                                                                        i
...
   Ответ: См. B3.
 ...
 B2 1.23   Найдите для каждого x значение σ.


Действуем аналогично A6.
 ...
 B3 4.10С помощью линейной интерполяции найдите, каким значениям λ соответствуют полученные значения σ. Поскольку n были вычислены в B1, мы
получаем искомую зависимость n(λ).
...
                                                  0.190   2514           1.5171   0.2419   478.6
 ...
 B4 0.49Постройте график n(λ) в удобном масштабе (кривая должна занимать большую часть графика по обеим осям, оси не обязательно должны
начинаться с нуля).
