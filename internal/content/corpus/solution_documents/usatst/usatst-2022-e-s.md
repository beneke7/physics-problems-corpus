...
In this part, you will launch a disk with mass M , radius R and moment of inertia I = βM R2 towards a
...
The parameters you specify must be in the following ranges:
    • 0.5 m/s ≤ initial speed ≤ 10 m/s.
    • −50 rad/s ≤ initial angular velocity ≤ 50 rad/s.
...
2022 Experimental Exam                                                                                          4
 ...
 final quantities be denoted by unprimed and primed variables, respectively, so that
                        ...
                        vx = v sin θ,     vy = v cos θ,     vx0 = v 0 sin θ0 ,   vy0 = v 0 cos θ0 .
 ...
 If the disk stops slipping against the wall by the end of the collision, then vx0 = Rω 0 , which implies
 ...
 Thus, the change in horizontal velocity is
                                    ...
                                    Jx         1+β (Rω − vx )
                         vx0 − vx =    =
                                     M        µvy (1 + c)           disk slips throughout,

 with a similar result for ω 0 − ω = (vx0 − vx )/βR.
...
2022 Experimental Exam                                                                                        5
 ...
 With that in mind, we can solve the problem as follows.
    ...
    • To extract c, we set ω = θ = 0 and v = 10 m/s. We then calculate c = v 0 /v and average the
      ...
      doesn’t affect the result.)
       Because v 0 and v are proportional, one could also vary v, plot v 0 versus v, and find the slope c
    ...
    • Continuing to set θ = 0, we experiment with large values of v and ω to see when the disk stops
      slipping during the collision. (It’s easy to identify this, because when the disk stops slipping, vx0
      is independent of v, while when it slips throughout, vx0 is independent of ω.)

    • We pick parameters where v and ω are large but slipping happens throughout, such as v = 8 m/s
      and ω = 50 rad/s, and compute
                                                     vx0
                                              µ=           .
                                                  v(1 + c)
       We again average the result across several trials, using the spread to estimate the uncertainty.

    • We pick parameters where slipping stops, such as v = 10 m/s and ω = 30 rad/s, and compute
 ...
 could try using fixing a nonzero negative θ and adjusting ω until ω 0 = ω, which occurs when the disk
...
2. (a) Find the initial position (x0 , y0 ) of the hidden disk.
   (b) Find the radius R of the hidden disk.
      (c) Find the mass M of the hidden disk and the coefficient of restitution c between the disks.
                                                      ...
                                                      Solution
  For all parts here, we always want to use v = 10 m/s. As mentioned above, this reduces the effect of
...
2022 Experimental Exam                                                                                         7
 ...
 (b) The easiest way to do this is to set θ = 0 and fire at s = y0 + ∆s, for various values of ∆s. A
     ...
     uncertainty of r in quadrature, so your final uncertainty in R can’t possibly be less than 0.001 m.
     ...
     We can’t disentangle the parameters M and c, so this part requires plotting a line. Note that
                                           v       1 m    1
                                              0
                                                =      +     .
                                          v−v     1+cM   1+c
     Thus, plotting v/(v − v 0 ) versus m gives a line with slope 1/(M (1 + c)) and intercept 1/(1 + c).
     (Note that a rebound angle θ0 ≈ 180◦ corresponds to a negative v 0 here.) A good final result is
