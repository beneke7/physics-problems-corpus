---
id: solution-document-apho-2024-e-s
source: apho
kind: official_solution_document
language: en
solution_type: official
source_pdf: cache/phoxiv/apho/2024_E_S.pdf
extraction_method: pdftotext-layout
mapped_problems: []
verification_status: flagged
provenance_note: "Source-provided solution document; preserved from cache/phoxiv/apho/2024_E_S.pdf."
---

Points: 20                                                             Time: 5.0 Hours



Part A: Alignment of the setup [Total Point = 1.0]


   A.1.
 [0.6pt]




             *Marking scheme:


              Weight at one end of the gyroscope arm                           [0.1pt]

              Weight at the longer end of the gyroscope arm                    [0.1pt]

               If 20 mm (diameter) x 2 mm (thickness) disk is chosen          [0.1pt]

               If 20 mm (diameter) x 3 mm (thickness) disk is chosen          [0.2pt]

               If 20 mm (diameter) x 4 mm (thickness) disk is chosen          [0.3pt]

               If 26 mm (diameter) x 4 mm (thickness) disk is chosen          [0.4pt]




APHO 2024                                                                   Page 1 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                        Time: 5.0 Hours



    A.2.
  [0.2pt]




             *Marking scheme:


               Gravitational force, mg included           [0.1pt]

               External force, F included                 [0.1pt]




APHO 2024                                              Page 2 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                        Time: 5.0 Hours



    A.3.
               (b) Unequal length of the arm                                               [0.0pt]
  [0.2pt]

               (e) Weight of the disk                                                      [0.2pt]


             remark: zero score will be given if student selects more than 2 choices.




APHO 2024                                                                               Page 3 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                         Time: 5.0 Hours


Part B: Effect of Spinning Speed [Total point = 6.5]


    B.1.
  [1.5pt]




               Proper table to record the measurement values                              [0.1pt]


               Record rotation period using multiple cycles

               [0.1] if the number of multiple cycle taken is less than 3

               [0.2] if the number of multiple cycle taken is less than 6

               [0.3] if the number of multiple cycle taken is more or equal to 6
                                                                                          [0.3pt]


               Repeating the experiment

               [0.1] if the experiment is repeated twice

               [0.2] if the experiment is repeated three times

               [0.3] if the experiment is repeated four times

               [0.4] if the experiment is repeated five times or more
                                                                                          [0.4pt]

               Calculate the average period of single cycle                               [0.1pt]

               Proper label of unit V, s, and rad/s                                       [0.3pt]

               Calculate the precession velocity                                          [0.1pt]

               Evenly exploring a range of voltages                                       [0.1pt]




APHO 2024                                                                               Page 4 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                        Time: 5.0 Hours



     B.2.
   [0.8pt]




               where Ti is precession period and T_bar is average of precession period.


                Correct standard deviation equation                                          [0.1pt]

                Correct standard error equation                                              [0.1pt]

                Correct calculation                                                          [0.5pt]




APHO 2024                                                                                 Page 5 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                    Time: 5.0 Hours



     B.3.
   [0.9pt]




                Label axis correctly with unit (0.1pt for each axis)                  [0.2pt]

                Proper scale used (0.1pt for each axis)                               [0.2pt]


                Chart plotted correctly

                [0.2] If proper exponential trendline drawn is within 20% of the
                                                                                      [0.2pt]
                answer

                [0.1] If exponential trendline drawn


                Error bar is included in the chart                                    [0.2pt]

                Chart occupy more than 70% of the area.                               [0.1pt]




APHO 2024                                                                          Page 6 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                      Time: 5.0 Hours




APHO 2024                                            Page 7 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                           Time: 5.0 Hours



     B.4.
   [0.2pt]
                y = 0.4243x + 1.7352
                                                                               Eq-1
                where y is precession period (s), x is voltage (V)


                Shown correct graph equation                                 [0.1pt]

                Shown the workout of getting the equation                    [0.1pt]




APHO 2024                                                                 Page 8 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                      Time: 5.0 Hours



     B.5.
   [1.0pt]




APHO 2024                                            Page 9 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                     Time: 5.0 Hours




                Obtained the linear equation - z = 2075x - 143.66

                where z is rotor speed (rpm), x is voltage (V)


                x = (z + 143.66) / 2075                                                 Eq-2

                Replace Eq-2 into Eq-1

                y = 0.4243x + 1.7352 = 0.4243 [(z + 143.66) / 2075] + 1.7352


                y=2.045e-4z+1.76476

                -> w = (2pi/y) = 2pi/(2.045e-4z+1.76476)                                 Eq-3

                where w is precession velocity.


                Label axis correctly with unit (0.1pt for each axis)                   [0.2pt]

                Proper scale used (0.1pt for each axis)                                [0.2pt]

                Chart plotted correctly                                                [0.2pt]

                Shown correct linear graph equation from graph                         [0.1pt]

                Shown the workout of getting Eq-2                                      [0.1pt]

                Replace Eq-2 into Eq-1 and obtained the new equation Eq-3              [0.2pt]




APHO 2024                                                                          Page 10 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                 Time: 5.0 Hours



  B.6(i)
                                                                         Eq-4
 [0.5pt]
                                                                         Eq-5       [0.1pt]


                                                                         Eq-6       [0.1pt]


              Relation of mass of ring and radius can be found through

                                                                         Eq-7

                                                                                    [0.1pt]

                                                                                    [0.1pt]




                                                                         Eq-8       [0.1pt]




APHO 2024                                                                       Page 11 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                      Time: 5.0 Hours



   B.6(ii)
  [0.2pt]


               Magnitude correct                        [0.1pt]

               Unit correct                             [0.1pt]




APHO 2024                                           Page 12 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                          Time: 5.0 Hours



  B.6(iii)
  [1.0pt]




                                                                            [1.0pt]




               *Correct diagram and proper label for mg, r, and L           [0.3pt]

                                                                            [0.1pt]

                                                                            [0.1pt]

               Precession angular velocity,


                                                                            [0.1pt]


                                                                            [0.1pt]


                                                                            [0.1pt]


                                                                            [0.2pt]




APHO 2024                                                               Page 13 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                               Time: 5.0 Hours



  B.6(iv)
  [0.4pt]




               Replacement in equation correct                   [0.1pt]

               Including mass of motor (0.089 + 0.094)           [0.1pt]

               Magnitude correct                                 [0.1pt]

               Unit correct                                      [0.1pt]




APHO 2024                                                    Page 14 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                      Time: 5.0 Hours


Part C: Influence of Gyroscope Arm Length [Total Point = 2.1]


   C.1.
 [1.2pt]




              *Proper table to record the measurement values                            [0.1pt]


              Record rotation period using multiple cycles

              [0.1] if the period of multiple cycle taken is less than 5
                                                                                        [0.3pt]
              [0.2] if the period of multiple cycle taken is between 5 to 10

              [0.3] if the period of multiple cycle taken is more than 10



              Number of data points collected

              [0.1] If the number of data points collected is between 3 and 5           [0.2pt]

              [0.2] If the number of data points collected is more than 5


              Calculate the average period of single cycle                              [0.1pt]

              Proper label of unit V, s, and rad/s                                      [0.3pt]

              Calculate the precession velocity                                         [0.2pt]




APHO 2024                                                                           Page 15 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                            Time: 5.0 Hours



    C.2.
  [0.8pt]




             *Marking


               Label axis correctly with unit (0.1pt for each axis)           [0.2pt]

               Proper scale used (0.1pt for each axis)                        [0.2pt]

               Chart plotted correctly                                        [0.2pt]

               Proper linear trendline drawn                                  [0.1pt]

               Chart occupy more than 70% of the graph area                   [0.1pt]




APHO 2024                                                                 Page 16 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                       Time: 5.0 Hours



    C.3.
  [0.1pt]    (a) Arm length increase, precession rate increase




APHO 2024                                                            Page 17 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                       Time: 5.0 Hours


Part D: Influence of Gyroscope Disk weight [Total point = 3.7]


   D.1.
 [1.7pt]




APHO 2024                                                            Page 18 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                      Time: 5.0 Hours



  [1.3pt]
               *Proper table to record the measurement values                           [0.1pt]


               Record rotation period using multiple cycles

               [0.1] if the period of multiple cycle taken is less than 5
                                                                                        [0.3pt]
               [0.2] if the period of multiple cycle taken is between 5 to 10

               [0.3] if the period of multiple cycle taken is more than 10



               Number of data points collected

               [0.1] If the number of data points collected is between 3
                                                                                        [0.3pt]
               [0.2] If the number of data points collected is more than 7

               [0.3] If the number of data points collected is more than 10


               Calculate the average period of single cycle                             [0.1pt]

               Proper label of unit V, s, and rad/s                                     [0.3pt]

               Calculate the precession velocity                                        [0.3pt]

               Table in all the value of 2mm, 3mm, and 4mm                              [0.3pt]




APHO 2024                                                                           Page 19 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                             Time: 5.0 Hours



     D.2.
   [1.1pt]




               *Marking


                Label axis correctly with unit (0.1pt for each axis)           [0.2pt]

                Proper scale used (0.1pt for each axis)                        [0.2pt]

                Chart plotted correctly                                        [0.3pt]

                Trendline drawn                                                [0.3pt]

                Chart occupy more than 70% of the graph area                   [0.1pt]




APHO 2024                                                                  Page 20 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                           Time: 5.0 Hours



     D.3.
   [0.1pt]     (b) The gyroscope will precess faster as the disk weight increased.




APHO 2024                                                                                Page 21 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                        Time: 5.0 Hours



     D.4.
   [0.8pt]     From B.6(iii), the precession angular velocity is given by


                                                                                           [0.1pt]



               where m is the mass of the disk and M is the mass of the motor. From B.6(i), the
               moment of inertia of a uniform disk is


                                                                                           [0.1pt]


               where R is the radius of the disk. Therefore,


                                                                                           [0.1pt]



                                                                                           [0.0pt]



                                                                                           [0.1pt]



               The mass of the motor M is given, g is the gravitational acceleration, ω is provided
                                                                                      s


               in B.5, r is the length of the arm fixed at 25cm, and ω is measured by the students.
                                                                      p




                Student calculate from m to m
                                            1    n                                         [0.2pt]

                Calculate average m                                                        [0.1pt]

                Calculate standard deviation                                               [0.1pt]




APHO 2024                                                                              Page 22 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                          Time: 5.0 Hours


Part E: Torque Induced by External Forces [Total point = 3.5]


        E.1.
      [1.3pt]




                   *Proper table to record the measurement values      [0.1pt]


                   Repeating record the data

                   [0.1] if the data is taken only once                [0.2pt]

                   [0.2] if the data repeated 3 times or more


                   Calculate the average period of single cycle        [0.1pt]

                   Calculate the standard deviation                    [0.1pt]

                   Calculate the standard error                        [0.1pt]

                   Calculate the tilted angle                          [0.1pt]

                   Proper label of unit V, s, and rad/s                [0.3pt]

                   Calculate the precession velocity                   [0.2pt]

                   Even bolts and nuts are added                       [0.1pt]




APHO 2024                                                               Page 23 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                              Time: 5.0 Hours



     E.2.
   [0.8pt]




                *Label axis correctly with unit (0.1pt for each axis)           [0.2pt]

                Proper scale used (0.1pt for each axis)                         [0.2pt]

                Chart plotted correctly                                         [0.1pt]

                Linear trendline drawn                                          [0.1pt]

                Shows error bar                                                 [0.1pt]

                Chart occupy more than 70% of the graph area                    [0.1pt]




APHO 2024                                                                   Page 24 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                            Time: 5.0 Hours



   E.3.
 [0.8pt]




              *Label axis correctly with unit (0.1pt for each axis)           [0.2pt]

              Proper scale used (0.1pt for each axis)                         [0.2pt]

              Chart plotted correctly                                         [0.1pt]

              Linear trendline drawn                                          [0.1pt]

              Shows error bar                                                 [0.1pt]

              Chart occupy more than 70% of the graph area                    [0.1pt]




APHO 2024                                                                 Page 25 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                           Time: 5.0 Hours



   E.4.
 [0.5pt]     From B.6(iii), the precession velocity is




             where N is the number of nuts and bolts, m is the mass of one set of nut and bolt, g
              is the gravitational acceleration, r is the length of the arm, I is the moment of inertia
             of the disk, ω is the angular speed of the disk, and ω is the angular speed of the
                            s                                          p


             precession.




              Calculated all the m values for different number of bolt and
                                                                                              [0.2pt]
              nut

              Calculated the average value of m                                               [0.1pt]

              Calculated the standard deviation                                               [0.1pt]

              Measure r from the experiment                                                   [0.1pt]




APHO 2024                                                                                 Page 26 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                      Time: 5.0 Hours



   E.5.
              I                                         [0.1pt]
 [0.1pt]




APHO 2024                                           Page 27 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                        Time: 5.0 Hours


Part F: Nutation phenomenon [Total point = 2.1]


   F.1.
 [0.4pt]




              (a) When the angle is lower than the horizontal.     [0.2 pt]

              (b) When the angle is higher than the horizontal.    [0.2 pt]




APHO 2024                                                             Page 28 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                        Time: 5.0 Hours



  F.2(i)
                                                         [0.1 pt]
 [1.0pt]

                                                         [0.1 pt]

                                                         [0.1 pt]

                                                Eq-4     [0.1 pt]

                                                Eq-5     [0.1 pt]

                                                         [0.1 pt]

                                                Eq-6

                                                Eq-7

                                                Eq-8     [0.1 pt]

                                                Eq-9     [0.1 pt]




                                                         [0.1 pt]


                                                Eq-10    [0.1 pt]




APHO 2024                                               Page 29 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                                 Time: 5.0 Hours



   F.2(ii)
  [0.1pt]                                                       [0.1 pt]




Part G: Application of gyroscope in self balancing [Total Point = 1.7]




APHO 2024                                                                      Page 30 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                  Time: 5.0 Hours



   G.1.
 [1.7pt]




              Correct setup                                         [0.2pt]

              Correct labeling of parts                             [0.3pt]

              Correct labeling of static and moving parts           [0.3pt]




APHO 2024                                                       Page 31 of 32
Experiment Marking Scheme, English (Official)

Points: 20                                                  Time: 5.0 Hours




              [0.2] Correct setup without 26 cm disk
                                                                    [0.3pt]
              [0.3] Correct setup with 26 cm disk


              Correct labeling of parts                             [0.3pt]

              Correct labeling of static and moving parts           [0.3pt]




APHO 2024                                                       Page 32 of 32
Experiment Marking Scheme, English (Official)
