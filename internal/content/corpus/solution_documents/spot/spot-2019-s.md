...
that the horizontal velocity u2 is a component of the tangential velocity. The angles are a bit tricky, especially
...
the Sun varies as 1/r2 , while the power emitted varies as T 4 .
...
The magnitude is 4.06 V m−1 and the angle is 38° from the edge a.
...
The term sin(ωt) cos(ωt) will go as 21 sin(2ωt), which is purely oscillatory, while the term sin2 (ωt) goes as
1
...
The |f~|2 term is basically 1/r2 in polar coordinates, so we can approximate the square lattice by a circle of
...
The idea is that the “vortex centre” can occupy roughly Ω = L2 positions because that is the number of
...
The idea is that when the lattice size is large, the ln L term is large. Thus the free energy will flip suddenly
