---
id: solution-document-ipho-2006-e1-s
source: ipho
kind: official_solution_document
language: en
solution_type: official
source_pdf: cache/phoxiv/ipho/2006/E1_S.pdf
extraction_method: pdftotext-layout
mapped_problems: [ipho-2006-e1]
verification_status: pending
provenance_note: "Source-provided solution document; preserved from cache/phoxiv/ipho/2006/E1_S.pdf."
---

The 37th International Physics Olympiad
               Singapore


Experimental Competition

  Wednesday, 12 July, 2006




    Sample Solution


                                                     Part 1
a. A sketch of the experimental setup (not required)

                                            Receiver


                                        Rotating table


                                                                               Goniometer
                                                                               Movable arm
                     Goniometer
                     Fixed arm            Beam
                                          splitter




                          Transmitter


                                                                      Holder



                                                                           Reflector




b. Data sheet (not required)
 Position    Meter           Position          Meter         Position          Meter         Position   Meter
  (cm)      reading            (cm)          reading          (cm)             reading        (cm)      reading
             (mA)                              (mA)                             (mA)                    (mA)
  104.0      0.609            100.9            1.016          96.0             0.514          91.0       0.925
  103.9      0.817            100.85           1.060          95.8             0.098          90.9       1.094
  103.8      0.933            100.8            1.090          95.6             0.192          90.8       1.245
  103.7      1.016            100.7            0.994          95.4             0.669          90.7       1.291
  103.6      1.030            100.6            0.940          95.3             0.870          90.6       1.253
  103.5      0.977            100.4            0.673          95.2             1.009          90.4       0.978
  103.4      0.890            100.2            0.249          95.1             1.119          90.2       0.462
  103.3      0.738            100.0            0.074          95.0             1.138          90.0       0.045
  103.2      0.548             99.8            0.457          94.9             1.080          89.8       0.278
  103.1      0.310             99.6            0.883          94.7             0.781          89.6       0.809
  103.0      0.145             99.4            1.095          94.5             0.403          89.5       1.031
  102.9      0.076             99.3            1.111          94.3             0.044          89.4       1.235
  102.8      0.179             99.2            1.022          94.1             0.364          89.3       1.277
  102.7      0.392             99.0            0.787          93.9             0.860          89.2       1.298
  102.6      0.623             98.8            0.359          93.7             1.103          89.1       1.252
  102.5      0.786             98.6            0.079          93.6             1.160          89.0       1.133
  102.4      0.918             98.4            0.414          93.5             1.159          88.8       0.684
  102.3      0.988             98.2            0.864          93.4             1.083          88.6       0.123
  102.2      1.026             98..0           1.128          93.2             0.753          88.5      ­0.020
  102.1      1.006             97.9            1.183          93.0             0.331          88.4       0.123
  102.0      0.945             97.8            1.132          92.8             0.073          88.2       0.679
  101.9      0.747             97.7            1.015          92.6             0.515          88.0       1.116
  101.8      0.597             97.5            0.713          92.4             0.968          87.9       1.265
  101.7      0.363             97.2            0.090          92.2             1.217          87.8       1.339
  101.6      0.161             97.0            0.342          92.15            1.234          87.7       1.313
  101.5      0.055             96.8            0.714          92.1             1.230          87.6       1.190
  101.4      0.139             96.6            1.007          92.0             1.165          87.4       0.867
  101.3      0.357             96.5            1.087          91.8             0.871          87.2       0.316



                                                         2


101.2   0.589                          96.4           1.070             91.6               0.353           87.1    0.034
101.1   0.781                          96.3           1.018             91.4               0.018           87.0   ­0.018
101.0   0.954                          96.2           0.865             91.2               0.394           86.9    0.178
104.0   0.609                          100.9          1.016             96.0               0.514           91.0    0.925
103.9   0.817                          100.8          1.060             95.8               0.098           90.9    1.094
103.8   0.933                          100.8          1.090             95.6               0.192           90.8    1.245
103.7   1.016                          100.7          0.994             95.4               0.669           90.7    1.291
103.6   1.030                          100.6          0.940             95.3               0.870           90.6    1.253
103.5   0.977                          100.4          0.673             95.2               1.009           90.4    0.978
103.4   0.890                          100.2          0.249             95.1               1.119           90.2    0.462
103.3   0.738                          100.0          0.074             95.0               1.138           90.0    0.045
103.2   0.548                          99.8           0.457             94.9               1.080           89.8    0.278
103.1   0.310                          99.6           0.883             94.7               0.781           89.6    0.809
103.0   0.145                          99.4           1.095             94.5               0.403           89.5    1.031
102.9   0.076                          99.3           1.111             94.3               0.044           89.4    1.235
102.8   0.179                          99.2           1.022             94.1               0.364           89.3    1.277
102.7   0.392                          99.0           0.787             93.9               0.860           89.2    1.298
102.6   0.623                          98.8           0.359             93.7               1.103           89.1    1.252
102.5   0.786                          98.6           0.079             93.6               1.160           89.0    1.133
102.4   0.918                          98.4           0.414             93.5               1.159           88.8    0.684
102.3   0.988                          98.2           0.864             93.4               1.083           88.6    0.123
102.2   1.026                          98.0           1.128             93.2               0.753           88.5   ­0.020
102.1   1.006                          97.9           1.183             93.0               0.331           88.4    0.123
102.0   0.945                          97.8           1.132             92.8               0.073           88.2    0.679
101.9   0.747                          97.7           1.015             92.6               0.515           88.0    1.116
101.8   0.597                          97.5           0.713             92.4               0.968           87.9    1.265
101.7   0.363                          97.2           0.090             92.2               1.217           87.8    1.339
101.6   0.161                          97.0           0.342             92.15              1.234           87.7    1.313
101.5   0.055                          96.8           0.714             92.1               1.230           87.6    1.190
101.4   0.139                          96.6           1.007             92.0               1.165           87.4    0.867
101.3   0.357                          96.5           1.087             91.8               0.871           87.2    0.316
101.2   0.589                          96.4           1.070             91.6               0.353           87.1    0.034
101.1   0.781                          96.3           1.018             91.4               0.018           87.0   ­0.018
101.0   0.954                          96.2           0.865             91.2               0.394           86.9    0.178



                                2.0


                                1.6
                                       87.8 cm

                                1.2                                                                 103.6 cm




           Meter reading (mA)
                                0.8



                                0.4


                                0.0


                                ­0.4

                                        88       90   92      94       96       98   100      102    104
                                                              position (cm)




                                                                   3


From the graph (not required) or otherwise, the positions of the first maximum point and
12th maximum point are measured at 87.8 cm and 103.6 cm.
The wavelength is calculated by
                 l       103.6 - 87.8
                     =                cm                                   1.8 marks
                 2           11
Thus,            l = 2.87 cm.


Error analysis
                                 2
                           l=      d,      Dd = 0.05 x2 cm = 0.1 cm.
                                11
                                    2      2                                 0.2 marks
                            Dl =      Dd = ¥ 0.10 = 0.018 cm < 0.02 cm
                                   11     11




                                                  4


                                             Part 2
(a)    Deduction of interference conditions

                                                               A


                                                                         B
                                    q1          q1




                                                                     n            t
                                           q2

Assume that the thickness of the film is t and refractive index n. Let q1 be the incident
angle and q 2 the refracted angle. The difference of the optical paths DL is:


                              DL = 2(nt / cos q 2 - t tan q 2 sin q1 )
Law of refraction:
                                         sin q 1 = n sin q 2
Thus

                                     DL = 2t n 2 - sin 2 q 1


Considering the 180 deg (p) phase shift at the air­ thin film interface for the reflected
beam, we have interference conditions:


                2t n 2 - sin 2 q min = ml (m = 1, 2, 3,...)         for the destructive peak
                                            1
and             2t n 2 - sin 2 q max = ( m ± )l                     for the constructive peak   1 mark
                                            2
If thickness t and wave length l are known, one can determine the refractive index of the
thin film from I ­ q 1spectrum (I is the intensity of the interfered beam).




                                                     5


      (b)      A sketch of the experimental setup



                                              Receiver



                                                              Goniometer
                              Plano­convex                    Movable arm
                              cylinder lens
                                                         θ   Rotating table


                                                    θ                     Thin film




                       Transmitter        Goniometer                                  1 mark
                                          Fixed arm



                              Students should use the labeling on Page 2.


(c)         Data Set

                 X: q1 / degree      Y: Meter reading S/mA

              40.0                   0.309
              41.0                   0.270
              42.0                   0.226
              43.0                   0.196
              44.0                   0.164
              45.0                   0.114
              46.0                   0.063
              47.0                   0.036
              48.0                   0.022
              49.0                   0.039
              50.0                   0.066
              51.0                   0.135
              52.0                   0.215
              53.0                   0.262
              54.0                   0.321
              55.0                   0.391
              56.0                   0.454
              57.0                   0.511


                                                         6


         58.0               0.566
         59.0               0.622
         60.0               0.664
         61.0               0.691
         62.0               0.722
         63.0               0.754
         64.0               0.796
         65.0               0.831
         66.0               0.836
         67.0               0.860
         68.0               0.904
         69.0               0.970
         70.0               1.022
         71.0               1.018
         72.0               0.926
         73.0               0.800
                                                                                                     0.5 marks
         74.0               0.770
         75.0               0.915
      Uncertainty: angle Dq1 = ±0.5 o , current: ±0.001 mA



                               1.2
                                                                                       o
                                                                                  70.5
                               1.0




          Meter reading (mA)
                               0.8


                               0.6


                               0.4


                               0.2


                               0.0                   o
                                                    48
                                     35   40   45        50   55        60   65   70       75   80
                                                                    o
                                                                  q( )


                                                                                                      0.9 marks
From the data, q min and q max can be found at 48o and 70.5o respectively.
To calculate the refractive index, the following equations are used:                                  0.6 marks



                                                              7


                         2t n 2 - sin 2 48 o = ml (m = 1, 2, 3,...)                 (1)
                                                      1
and                      2t n 2 - sin 2 70.5 o = ( m - )l                           (2)
                                                      2
In this experiment, t = 5.28 cm, l = 2.85cm (measured using other method).
Solving the simultaneous equations (1) and (2), we get
                               sin 2 70.5 o - sin 2 48 o
                         m=                                   + 0.25
                                            l 2
                                        (        )
                                            2t
                                                                                                       1 mark
                         m = 4.83                           m=5
Substituting m = 5 in (1), we get n = 1.54
Substituting m = 5 in (2), we also get n = 1.54                                                       0.5 marks




Error analysis:
                                                            ml 2
                                  n = sin 2 q + (              )
                                                            2t
                                            1                                m 2l       m 2 l2
                         Dn =                               (sin 2q • Dq +        D l -        Dt )
                                                     ml 2                    2t 2        2t 3
                                  sin 2 q + (           )
                                                     2t
                             1               m 2l    m 2 l2
                         =     (sin 2q • Dq + 2 Dl -        Dt )
                             n               2t       2t 3
If we take Dq = ±0.5o = ±0.0087 rad, Dt = ±0.05 cm, Dl = ±0.02 cm, and q = 48o


                 1                     5 2 ¥ 2.85           5 2 ¥ 2.85 2
        Dn =        (0.0087 sin 96 o +            ¥ 0 .01 +              ¥ 0.05) ª 0.02
               1.54                    2 ¥ 5.28 2            2 ¥ 5.283

Thus,                    n + Dn = 1.54 ±0.02                                                          0.5 marks




                                                        8


                                               Part 3

                                       Sample Solution
    Task 1
    Sketch your final experimental setup and mark all components using the labels given at
    page 2. In your sketch, write down the distance z (see Figure 3.2), where z is the distance
    from the tip of the prism to the central axis of the transmitter.

           Transmitter




                                              z
                          Lens                                 Prism

                                               Prism



                                                                  d             Receiver


                              (Students should use labels on page 2.)

    Task 2
    Tabulate your data. Perform the experiment twice.

    Data Set
                     Set 1     Set 2    Saverage
X: d(cm)   DX(cm)                                      DS(mA)#    It (mA)2*    D(It)$   Y: ln(It (mA) 2)   DY&
                    S1 (mA)   S2(mA)    (mA)
  0.60      0.05      0.78     0.78     0.780           0.01          0.6080   0.016         ­0.50         0.03
  0.70      0.05      0.68     0.69     0.685           0.01          0.4690   0.014         ­0.76         0.03
  0.80      0.05      0.58     0.59     0.585           0.01          0.3420   0.012         ­1.07         0.03
  0.90      0.05      0.50     0.51     0.505           0.01          0.2550   0.010         ­1.37         0.04
  1.00      0.05      0.42     0.42     0.420           0.01          0.1760   0.008         ­1.74         0.05
  1.10      0.05      0.36     0.35     0.355           0.01          0.1260   0.007         ­2.07         0.06
  1.20      0.05      0.31     0.31     0.310           0.01          0.0961   0.006         ­2.34         0.06
  1.30      0.05      0.26     0.25     0.255           0.01          0.0650   0.005         ­2.73         0.08
  1.40      0.05      0.21     0.22     0.215           0.01          0.0462   0.004         ­3.07         0.09


    #
      DS = 0.01 mA (for each set of current measurements)
    *
      S2 proportional to the intensity, It
    $
      D(S2) = DIt = 2 S ¥ DS
    &
      DY = D(lnIt) = D(It)/It

                                                   9


Task 3
By plotting appropriate graphs, determine the refractive index, n1, of the prism with error
analysis. Write the refractive index n1, and its uncertainty ∆n1, of the prism in the answer
sheet provided.




Least Square Fitting

          X = d(cm)   DX(cm)   Y = ln(It)   DY        DY2       XY        X2      Y2
            0.60       0.05     ­0.50       0.03     0.001     ­0.298    0.360   0.247
            0.70       0.05     ­0.76       0.03     0.001     ­0.530    0.490   0.573
            0.80       0.05     ­1.07       0.03     0.001     ­0.858    0.640   1.150
            0.90       0.05     ­1.37       0.04     0.002     ­1.230    0.810   1.867
            1.00       0.05     ­1.74       0.05     0.002     ­1.735    1.000   3.010
            1.10       0.05     ­2.07       0.06     0.003     ­2.278    1.210   4.290
            1.20       0.05     ­2.34       0.06     0.004     ­2.811    1.440   5.487
            1.30       0.05     ­2.73       0.08     0.006     ­3.553    1.690   7.469
            1.40       0.05     ­3.07       0.09     0.009     ­4.304    1.960   9.451

            SX =                SY =        SDY =   S(DY)2 =   SXY =     SX2 =   SY2 =
            9.00               ­15.648      0.469    0.029     ­17.596   9.600   33.544




                                               10


From I t = I 0 exp ( -2g d ) , taking natural log on both sides, we obtain:

                               ln( I t ) = -2g d + ln( I 0 )

which is of the form y = mx + c.


To calculate the gradient, the following equation was used, where N is the number of data
points:

                             N Â ( XY ) - ( Â X )( Â Y )
                        m=                                       = -3.247
                                   N Â X 2 - (Â X )
                                                         2




To calculate the standard deviation sY of the individual Y data values, the following
equation was used:


                                s = Â
                                      ( DY )
                                                     2

                                   Y         = 0.064
                                            N -2

Hence the standard deviation in the slope can be calculated:


                                                N
                          s m = sY                               = 0.082
                                       N Â X 2 - (Â X )
                                                             2




From the gradient:
                                        2g = 3.247 ± 0.082
                                           ª 3.25 ± 0.08

Using:
                                                k22 + g 2
                                        n1 =
                                               k2 sin q1


where q1 = 60o, k2 = 2p/l ª 2.20 (using the wavelength determined from earlier part
(using l = (2.85 ± 0.02)cm), we obtain:

                                        n1 ± Dn1 = 1.434 ± 0.016
                                                    ª 1.43 ± 0.02


                                               11


Error Analysis for refractive index of n1

                                 È k 2 + g 2 12 ˘          È k 2 + g 2 12 ˘
                       Dn1 =
                              d Í 2 (       )   ˙ Dk + d Í ( 2        ) ˙ Dg
                             dk2 Í k2 sin q1 ˙         d g Í k2 sin q1 ˙
                                                    2

                                 ÍÎ             ˙˚         ÍÎ             ˙˚

                             È k 2 + g 2 - 12              2 ˘    È g k 2 + g 2 - 12 ˘
                                ( 2 ) - ( 2 ) ˙ Dk + Í ( 2 ) ˙ Dg
                                                         1
                                              k 2
                                                  + g 2

                       Dn1 = Í
                             Í sin q1          k22 sin q1 ˙
                                                                2
                                                                  Í k2 sin q1 ˙
                             ÍÎ                              ˙˚   ÍÎ                 ˙˚
                           = 0.016
                           ª 0.02


where:
                                            2p
                                 Dk2 = -         Dl = -0.015
                                            l2

Note: Other methods of error analysis are also accepted.




                                                 12


                                         Part 4


Task 1

Top­view of a simple square lattice.

                   a



                            d




                                                                                 0.5 marks


Figure 4.1: Schematic diagram of a simple square lattice with lattice constant a and
interplaner d of the diagonal planes indicated.



Deriving Bragg's Law

Conditions necessary for the observation of diffraction peaks:

   1. The angle of incidence = angle of scattering.
   2. The pathlength difference is equal to an integer number of wavelengths.




                                            13


                 Figure 4.2: Schematic diagram for deriving Bragg's law.


h = d sinq                                    (1).

The path length difference is given by,

2h = 2d sinq                                  (2).

For diffraction to occur, the path difference must satisfy,

2 d sinq = ml,         m = 1, 2, 3...         (3).




                                              a
                                                  d



                                                                                       0.5 marks


          Figure 4.3 Illustration of the lattice used in the experiment (this Figure
          is not required)




                                             14


Fig. 4.4 Actual lattice used for the
experiment (this Figure is not required)




             Fig. 4.4 The actual lattice used in the experiment (not required)


Task 2 (a)


                        Lattice Box I on
                        Rotating Table L              Plano­cylindrical
                                                      Lens on Holder
                              I
                                                       J        N
                                                                         A
                  q
 Digital
 Multimeter                                                           Microwave
                                                                      Transmitter on
                                                                      Holder
   D
                                           z = 180° ­ 2q



                             B
                                   Microware
                                   Receiver
                                   on Holder

                                                                                 1.5 marks
                        Fig. 4.5 Sketch of the experimental set up


                                             15


Task 2(b) & 2(c)

Data Set

                    Output    Intensity
                                    2
q/°         z/°     current      I=S
                                     2
                    S (mA)      (mA)                                    0.14
20.0       140.0     0.023    0.000529
21.0       138.0     0.038    0.001444                                  0.12
22.0       136.0     0.070     0.0049
23.0       134.0     0.109    0.011881
                                                                         0.1


                                                                    2


                                            Relative

                                            Relative intensity
                                                     Intensity (mA)
24.0       132.0     0.163    0.026569
25.0       130.0     0.201    0.040401



                                                               (mW)
                                                                        0.08
26.0       128.0     0.233    0.054289
27.0       126.0     0.275    0.075625
                                                                        0.06
28.0       124.0     0.320     0.1024
29.0       122.0     0.350     0.1225
                                                                        0.04
30.0       120.0     0.353    0.124609
31.0       118.0     0.358    0.128164
                                                                        0.02
32.0       116.0     0.354    0.125316
33.0       114.0     0.342    0.116964
                                                                          0
34.0       112.0     0.321    0.103041
                                                                               16 18 20 22 24 26 28 30 32 34 36 38 40 42 44 46 48 50 52 54 56
35.0       110.0     0.303    0.091809
                                                                                                       Theta (degree)
36.0       108.0     0.280     0.0784
37.0       106.0     0.241    0.058081
38.0       104.0     0.200       0.04
39.0       102.0     0.183    0.033489
40.0       100.0     0.162    0.026244
41.0        98.0     0.139    0.019321
42.0        96.0     0.120     0.0144
43.0        94.0     0.109    0.011881
44.0        92.0     0.086    0.007396
45.0        90.0     0.066    0.004356
46.0        88.0     0.067    0.004489
47.0        86.0     0.066    0.004356
48.0        84.0     0.070     0.0049
49.0        82.0     0.084    0.007056
                                                                                                                   2.7 marks
50.0        80.0     0.080     0.0064



Task 2(d)

From eq 3 and let m = 1,

 2d sin q max = l                         (4)

From Fig. 4.3,



                                                                        16


a = 2d                              (5)

Combine eqs (4) and (5), we obtain,

                   l
        a=
                2 sin q max

From the symmetry of the data, the peak position is determined to be:

qmax = 31°      (The theoretical value is qmax = 32°)


                   l              2.85cm
        a=                    =             = 3.913cm
                2 sin q max       2 sin 31o

 (Actual value a = 3.80 cm)

   [The value 3.55 in the marking scheme is derived from:
             l          2.83cm
   a=                =             = 3.58cm
         2 sin q max    2 sin 34 o

   where 2.83 cm and 34 deg are the min and max allowed values for wavelength and
   peak position.

   Similarly:

                                                  l          2.87cm
   The value 4.10 is derived from: a =                   =              = 4.06cm
                                               2 sin q max   2 sin 30 o
                                                   l         2.83cm
   The value 3.55 is derived from: a =                     =            = 3.58cm
                                               2 sin q max   2 sin 34 o

                                                  l          2.83cm
   The value 3.40 is derived from: a =                   =              = 3.49cm
                                               2 sin q max   2 sin 35 o
                                                   l         2.87cm
   The value 4.20 is derived from: a =                     =            = 4.18cm ]
                                               2 sin q max   2 sin 29 o

Error analysis:

Known uncertainties:
∆λ = 0.02 cm;
∆q = 0.5 deg = 0.014 rad. (uncertainty in determining q from graph).



                                                 17


                            l
From:        a=
                      2 sin q max
                 Dl                  l          d
Da =                        -                     (sin q max ) Dq
            2 sin q max         2 (sin q max ) dq
                                              2


       Dl             1     d
= a(         -                (sin q max ) Dq )
      l          sin q max dq
     Dl
= a(    - cot q max Dq )
      l
       0.02
= 3.80(     - cot(32°) ¥ ( -0.014))cm
       2.85
= 0.112cm ª 0.1

Hence:                                                              0.8 marks
                          a ± ∆a = 3.913 ± 0.112
                                 ª 3.9 ± 0.1 cm




                                                          18
