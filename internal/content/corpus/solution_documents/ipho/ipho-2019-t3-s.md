---
id: solution-document-ipho-2019-t3-s
source: ipho
kind: official_solution_document
language: en
solution_type: official
source_pdf: cache/phoxiv/ipho/2019/T3_S.pdf
extraction_method: pdftotext-layout
mapped_problems: [ipho-2019-t3]
verification_status: pending
provenance_note: "Source-provided solution document; preserved from cache/phoxiv/ipho/2019/T3_S.pdf."
---

Theory IPhO 2019                                                                                Q1-1
                                                                                               S3-1
Thermoacoustic engine – Solution
Part A: Sound wave in a closed tube
                                                                                          2𝜋
A.1. The boundary conditions are: 𝑢(0, 𝑡) = 𝑢(𝐿, 𝑡) = 0. As a result, sin ( 𝜆 𝐿) = 0, so we get
𝜆max = 2𝐿.
A.2. We get

          𝑉(𝑥, 𝑡) = 𝑆 ⋅ (Δ𝑥 + 𝑢(𝑥 + Δ𝑥, 𝑡) − 𝑢(𝑥, 𝑡)) = 𝑆Δ𝑥 ⋅ (1 + 𝑢′ ) = 𝑉0 + 𝑉0 𝑢′.

Thus,
              𝑉(𝑥, 𝑡) = 𝑉0 + 𝑎𝑘𝑉0 cos(𝑘𝑥) cos(𝜔𝑡)               ⇒          𝑉1 (𝑥) = 𝑎𝑘𝑉0 cos(𝑘𝑥).
A.3. We use Newton’s Second Law 𝜌0 𝑢̈ = −𝑝′ to deduce 𝑝′ = −𝜌0 𝑢̈ = 𝜌0 𝑎𝜔2 sin(𝑘𝑥)cos(𝜔𝑡),
so that
                                𝜔2                                                   𝜔2
             𝑝(𝑥, 𝑡) = 𝑝0 − 𝑎 𝑘 𝜌0 cos(𝑘𝑥)cos(𝜔𝑡)               ⇒          𝑝1 (𝑥) = 𝑎 𝑘 𝜌0 cos(𝑘𝑥).

                                 𝑝 (𝑥)        𝑉 (𝑥)                 𝜌 𝜔2                       𝛾𝑝
A.4. Using 𝑎 ≪ 𝐿, we obtain 1𝑝            = 𝛾 1𝑉 . As a result, 𝑝0 𝑘 = 𝛾 ⋅ 𝑘, and 𝑐 = √ 𝜌 0.
                                     0          0                   0                           0


A.5. The relative change in 𝑇(𝑥, 𝑡) is the sum of the relative changes in 𝑉(𝑥, 𝑡) and 𝑝(𝑥, 𝑡). As a
result,
                      𝑇              𝑇                     𝑇
              𝑇1 (𝑥) = 0 𝑝1 (𝑥) − 0 𝑉1 (𝑥) = (𝛾 − 1) 0 𝑉1 (𝑥) = 𝑎𝑘(𝛾 − 1)𝑇0 cos(𝑘𝑥).
                      𝑝0             𝑉0                    𝑉0

A.6. The movement of the gas parcels inside the tube conveys heat along its boundary. To
determine the direction of the convection, we combining the result of Task A.5 and the
                                                                𝐿
expression (1) for 𝑢(𝑥, 𝑡). We see that when 0 < 𝑥 < 2, the gas is colder when the displacement
                                          𝐿
𝑢(𝑥, 𝑡) is positive. Likewise, when 2 < 𝑥 < 𝐿, the gas is colder when the displacement 𝑢(𝑥, 𝑡) is
negative. Hence, heat flows into the gas near the point B, cooling it down, and out of the gas near
the points A and C, heating them up.

Part B: Sound wave amplification induced by external thermal contact
B.1. We get
                                                                            𝜏
                           𝑇env (𝑡) = 𝑇plate (𝑥0 + 𝑢(𝑥0 , 𝑡)) = 𝑇0 − ℓ ⋅ 𝑢(𝑥0 , 𝑡),
    Theory IPhO 2019                                                                                  Q1-1
                                                                                                 S3-2
so that:
                                                     𝑎𝜏                    𝑎𝜏
                                               𝑇st = ℓ sin(𝑘𝑥0 ) = ℓ√2.

B.2. The gas will convey heat from the hot reservoir to the cold one if the parcels are colder than
the environment when 𝑢(𝑥0 , 𝑡) < 0, and hotter when 𝑢(𝑥0 , 𝑡) > 0. This occurs precisely if
                                                        𝑇st > 𝑇1.
Plugging in the results of Tasks A.5 and B.1, we get
               𝑎𝜏cr
                       sin(𝑘𝑥0 ) = 𝑎𝑘(𝛾 − 1)𝑇0 cos(𝑘𝑥0 )                   ⇒    𝜏cr = 𝑘ℓ(𝛾 − 1)𝑇0 .
                   ℓ

B.3. Using the first law of thermodynamics, we get
                                                   𝑑𝑄     𝑑𝐸        𝑑𝑉
                                                        = 𝑑𝑡 + 𝑝 𝑑𝑡 .
                                                   𝑑𝑡
                                     1
Plugging in the relation 𝐸 = 𝛾−1 𝑝𝑉, we see that:

             𝑑𝑄         1   𝑑                 𝑑𝑉    1     𝑑𝑝       𝛾       𝑑𝑉   1    𝑑𝑝     𝛾    𝑑𝑉
                   = 𝛾−1 𝑑𝑡 (𝑝𝑉) + 𝑝 𝑑𝑡 = 𝛾−1 𝑉 𝑑𝑡 + 𝛾−1 𝑝 𝑑𝑡 ≈ 𝛾−1 𝑉0 𝑑𝑡 + 𝛾−1 𝑝0 𝑑𝑡 .
              𝑑𝑡

                                         𝑑𝑄
B.4. We plug the expression for 𝑑𝑡 into the result of Task B.3. This gives:
                                 1    𝑑𝑝    𝛾     𝑑𝑉
                                   𝑉     + 𝛾−1 𝑝0 𝑑𝑡 = 𝛽𝑉0 (𝑇st − 𝑇1 ) ⋅ cos(𝜔𝑡).
                                𝛾−1 0 𝑑𝑡

We now plug in the data given in equation (6), and get (by considering terms with cos(𝜔𝑡) and
sin(𝜔𝑡) separately):
                                  1             𝛾
                                     𝑉0 𝑝𝑎 𝜔 +    𝑝 𝑉 𝜔 = 𝛽𝑉0 (𝑇st − 𝑇1 )
                                 𝛾−1           𝛾−1 0 𝑎
                                          1             𝛾
                                             𝑉0 𝑝𝑏 𝜔 −    𝑝 𝑉𝜔=0
                                         𝛾−1           𝛾−1 0 𝑏
and thus, we can already express 𝑉𝑏 as
                                                           1        𝑉
                                                    𝑉𝑏 = 𝛾 𝑝𝑏 ⋅ 𝑝0 .
                                                                       0


For 𝑉𝑎 , we plug in the results of Tasks B.1 and B.2,
                                                               𝑎
                                               𝑇st − 𝑇1 = ℓ√2 (𝜏 − 𝜏cr ),
    Theory IPhO 2019                                                                                                            Q1-1
                                                                                                                         S3-3
giving:

                                                    1             𝛾−1 𝛽 𝑎                                    𝑉
                               𝑉𝑎 = (− 𝛾 𝑝𝑎 − 𝛾 𝜔 ℓ√2 (𝜏 − 𝜏cr )) ⋅ 𝑝0 .
                                                                                                             0


B.5. We want to integrate the mechanical work generated, ∫ 𝑝𝑑𝑉, and averaging the result over
a long time. To do this, we substitute our expressions (6) for the perturbed 𝑝 and 𝑉. Since the
                                                                                                                     1
average of cos(𝜔𝑡)sin(𝜔𝑡) is 0, and that of sin2 (𝜔𝑡) and cos 2 (𝜔𝑡) is 2, we get:
                                           𝑉0
                                                𝑊tot = −𝜋 ⋅ (𝑝𝑎 𝑉𝑏 + 𝑝𝑏 𝑉𝑎 ).
                                           𝑆ℓ

Using the result of B.4, we get
                                𝑉0                      𝜋       𝛾−1        𝑎                             𝑝
                                     𝑊tot = 𝜔 ⋅ 𝛾 𝛽 ℓ√2 (𝜏 − 𝜏cr ) ⋅ 𝑉0 𝑝𝑏.
                                𝑆ℓ                                                                           0

                                                                                                                 𝜔2                1
To leading order, 𝑝𝑏 is the unperturbed wave 𝑝𝑏 ≈ 𝑝1 (𝑥0 ) = 𝑎 𝑘 𝜌0 cos(𝑘𝑥0 ) = 𝑎𝑘𝛾𝑝0                                                  .
                                                                                                                                   √2
Simplifying, we get
                        𝜋      𝛾−1              𝑎                         𝑝            𝜋
                𝑊tot = 𝜔 𝑆 ⋅ 𝛾 𝛽                    (𝜏 − 𝜏cr ) ⋅ 𝑏 =                        (𝛾 − 1)𝛽(𝜏 − 𝜏cr )𝑘𝑎2 𝑆.
                                            √2                            𝑝0           2𝜔

B.6. We want to compute the amount of heat convection over one cycle. This means that we
need to take the amount of heat moving in or out of the parcel, and weigh it by the position of
the parcel at that time. Thus, the total heat conveyed by the parcel, integrated along a cycle, is:
                                                                      1       𝑑𝑄
                                                    𝑄tot =                ∫ 𝑑𝑡 𝑢 ⋅ 𝑑𝑡.
                                                                  Δ𝑥
                                                                                               𝑑𝑄
This expression can be computed to leading order using 𝑑𝑡 = 𝛽𝑉0 (𝑇st − 𝑇1 ) ⋅ cos(𝜔𝑡) and the
                                                        𝑎
unperturbed displacement 𝑢(𝑥0 , 𝑡) =                         cos(𝜔𝑡). This gives
                                                        √2

                   𝜋                            𝑎           𝜋              𝑎                        𝑎            𝜋       𝑎2 𝑆
            𝑄tot = 𝜔 𝛽𝑉0 (𝑇st − 𝑇1 )                = 𝜔 𝛽𝑉0 ⋅ ℓ√2 (𝜏 − 𝜏cr ) ⋅                           = 2𝜔 𝛽(𝜏 − 𝜏cr ) ℓ .
                                            √2                                                      √2

B.7. Dividing the results of Tasks B.5 and B.6, we obtain the expression:
                                 𝑊                                             𝜏           𝜏    𝜏        𝜏
                            𝜂 = 𝑄 tot = (𝛾 − 1)𝑘ℓ = 𝑇cr = 𝜏cr ⋅ 𝑇 = 𝜏cr ⋅ 𝜂𝑐 .
                                     tot                                           0            0
