---
id: solution-document-ipho-2011-t1-s
source: ipho
kind: official_solution_document
language: en
solution_type: official
source_pdf: cache/phoxiv/ipho/2011/T1_S.pdf
extraction_method: pdftotext-layout
mapped_problems: [ipho-2011-t1]
verification_status: pending
provenance_note: "Source-provided solution document; preserved from cache/phoxiv/ipho/2011/T1_S.pdf."
---

Theoretical Competition:                 Solution
                                                                                     Question 1   Page 1 of 7


I. Solution


                                                                       

                                                                  2
                                                         1
                                               r1                          r2



                                       1                             2
                                 M                                               m
                                           R O                r




1.1 Let O be their centre of mass. Hence
                      MR  mr  0                                                       ……………………… (1)

                                  GMm
                      m02 r 
                                 R  r
                                           2

                                                                                        ……………………… (2)
                                   GMm
                      M R 
                          2

                                  R  r
                          0                    2




                                                    G  M  m
From Eq. (2), or using reduced mass, 02 
                                                     R  r
                                                                  3



         2   G(M m )             GM                  Gm
Hence,   0
                                                            .                    ……………………………… (3)
              (R r )3         r (R r )2            R(R r )2




                                                                                                           1
                                                          Theoretical Competition:      Solution
                                                                     Question 1       Page 2 of 7

1.2 Since  is infinitesimal, it has no gravitational influences on the motion of neither M nor
m . For  to remain stationary relative to both M and m we must have:
        GM          Gm                  G  M  m 
             cos 1  2 cos 2  02                                      ……………………… (4)
                                            R  r
           2                                        3
         r1           r2
                          GM           Gm
                             2
                               sin 1       sin  2                          ……………………… (5)
                           r1            r22
             GM
Substituting       from Eq. (5) into Eq. (4), and using the identity
              r12
sin 1 cos2  cos1 sin 2  sin(1  2 ) , we get

                     sin(1   2 )  M  m 
                 m                             sin 1                       ……………………… (6)
                                     R  r
                                             3
                          r22
The distances r2 and  , the angles 1 and  2 are related by two Sine Rule equations

                          sin 1       sin 1
                                   
                                        R
                                                                              ……………………… (7)
                           sin 1 sin 1   2 
                                 
                             r2       Rr
Substitute (7) into (6)

                            1
                               
                                  R        M  m
                                                                              ……………………… (10)
                           r2  R  r 
                             3          4
                                             m
         m       R
Since             ,Eq. (10) gives
      M m Rr
                            r2  R  r                            ……………………… (11)
                Gm
By substituting 2 from Eq. (5) into Eq. (4), and repeat a similar procedure, we get
                r2
                            r1  R  r                            ……………………… (12)

                                r1           R                r2      r
Alternatively,                                   and             
                          sin 180      sin 1           sin    sin  2
                          sin 1   R r              m r2
                                    2             
                          sin  2  r r1             M r1

Combining with Eq. (5) gives r1  r2


                                                                                                  2
                                                          Theoretical Competition:    Solution
                                                                       Question 1    Page 3 of 7

Hence, it is an equilateral triangle with
                                  1  60
                                                                          ……………………… (13)
                                   2  60

The distance  is calculated from the Cosine Rule.
                 2  r 2  ( R  r )2  2r ( R  r ) cos 60
                                                                          ……………………… (14)
                  r  rR  R
                         2          2


Alternative Solution to 1.2

Since  is infinitesimal, it has no gravitational influences on the motion of neither M nor
m .For  to remain stationary relative to both M and m we must have:
       GM             Gm                        G  M  m 
              cos           cos      2
                                                                  ……………………… (4)
                                                     R  r
                   1                2                        3
         r12            r22
                         GM              Gm
                             2
                                sin 1          sin  2                  ……………………… (5)
                           r1              r22
                               r1              R
Note that                                
                         sin 180       sin 1
                           r2        r
                                                   (see figure)
                         sin      sin  2
                         sin 1     R r            m r2
                                   2                                  ……………………… (6)
                         sin  2    r r1           M r1

Equations (5) and (6):                     r1  r2                        ……………………… (7)
                                           sin 1    m
                                                                         ……………………… (8)
                                           sin  2   M
                                           1   2                       ……………………… (9)

The equation (4) then becomes:

                         M cos 1  m cos  2 
                                                      M  m r 2 
                                                                          ……………………… (10)
                                                     R  r
                                                             3 1



                                               M  m r12 
Equations (8) and (10): sin 1   2                      sin  2      ……………………… (11)
                                                M  R  r 3
                                             r
Note that from figure,                                                   ……………………… (12)
                                  sin 2   sin  2



                                                                                              3
                                                          Theoretical Competition:                   Solution
                                                                            Question 1           Page 4 of 7

                                                M  m r12 r
Equations (11) and (12): sin 1   2                      sin 2              ……………………… (13)
                                                 M  R  r 3
Also from figure,
         R  r   r22  2r1r2 cos 1  2   r12  2r12 1  cos 1  2 
                2
                                                                                         ……………… (14)
                                                         sin 2
Equations (13) and (14): sin 1   2                                          ……………………… (15)
                                                2 1  cos 1  2  
               1  2  180  1  2  180  2 2                (see figure)
                       1
            cos 2  ,  2  60 ,  1  60
                       2

Hence M and m from an equilateral triangle of sides  R  r 
Distance  to M is R  r
Distance  to m is R  r
                                                                    2
                                 Rr                 3 
                                           2

Distance  to O is                  R    R  r                   R 2  Rr  r 2
                                 2                  2 


1.3 The energy of the mass        is given by

       GM  Gm 1             d
E                  2  (( )2   2 2 )                         ………………………..(15)
         r1      r2           dt
Since the perturbation is in the radial direction, angular momentum is conserved
( r1 r2       and m M ),
      2GM  1  d  2 0 40 2 
E         2  ( )                                                           ………………………..(16)
                 dt   2 
Since the energy is conserved,
dE
    0
 dt
dE 2GM  d           d  d 2     0 40 2 d 
                                            0 ……………(17)
 dt     2 dt          dt dt 2        3 dt

d d d  d  
                                              …………….(18)                                              
dt   d  dt   dt 
dE 2GM  d     d  d 2    0 40 2 d 
                                    0 …………….(19)                                               60o
dt   3   dt    dt dt 2        3 dt
                                                                                             R   O   R
                                                                                                             4
                                                              Theoretical Competition:           Solution
                                                                              Question 1        Page 5 of 7

      d
Since      0 , we have
      dt
2GM       d 2  0 40 2
                       0 or
 3       dt 2     3
d 2     2GM       0 40 2
                        .                                                           …………………………(20)
dt 2      3         3
                                                                   
The perturbation from 0 and  0 gives   0  1      and    0  1 
                                                                            0 
                                                                                  .
                                                   0              

Then
d 2 d 2                         2GM                               0 40 2
          ( 0   )                       0 1                                    ………………(21)
 dt 2 dt 2                         
                                             3
                                                           0           
                                                                                    3

                             30 1                               03 1     
                                     0                                   0 

Using binomial expansion (1   )n  1  n ,
d 2     2GM    3               2     3 
         3 0 1      1       00 1       .                                    ……………….(22)
 dt  2
           0        0     0               0 
              
Using           ,
              
d 2     2GM   30           2     3 
         3 0 1           00 1      .                                          ……………….(23)
 dt  2
           0       0   0 
                           2
                                           0 
              2GM
Since 02         ,
               30
d 2              30             3 
        02 0 1           0 0 1 
                                   2
                                                                                          ……………….(24)
 dt  2
                     0   0 
                            2
                                            0 
d 2             4 30  
        02 0                                                                        ……………….(25)
                  0   02 
     2
 dt
d 2                 302 
         2
                4                                                                     ……………….(26)
                       02 
           0
 dt 2            
                                        02           3
From the figure, 0  0 cos30 or                     ,
                                        0    2
                                                      4
d 
  2
                    9    7
       02   4     02  .                                                       …………….…(27)
                    4
    2
 dt                        4



                                                                                                         5
                                                      Theoretical Competition:             Solution
                                                                  Question 1             Page 6 of 7


                                          7
Angular frequency of oscillation is         0 .
                                         2

Alternative solution:
                                   G( M  M ) GM
M  m gives R  r and 02                           . The unperturbed radial distance of  is
                                    ( R  R )3   4 R3
  3R , so the perturbed radial distance can be represented by 3R   where   3R as
shown in the following figure.
                                    2GM                          d2
Using Newton’s 2 nd law,  2                       ( 3R   )        ( 3R   )   2 ( 3R   ) .
                            {R  ( 3R   ) }2 3/2                   2
                                                                  dt
(1)
The conservation of angular momentum gives 0 ( 3R)2  ( 3R   )2 .
(2)
Manipulate (1) and (2) algebraically, applying  2  0 and binomial approximation.
            2GM                            d 2    0 2 3R
                            ( 3 R   )       
    {R 2  ( 3R   ) 2 }3/2               dt 2 (1   / 3R)3
          2GM                     d 2    0 2 3R
                     ( 3R   )  2 
  {4 R 2  2 3 R}3/2             dt   (1   / 3R)3
    GM         (1   / 3R)     d 2      0 2 3R
        3 R                        
    4 R3     (1  3 / 2 R)3/2 dt      (1   / 3R)3
                                   2


            3 3               d 2               3 
0 2 3R 1            1       2  0 2 3R 1      
               4R            3R  dt                 3R 

         02  
d2           7
            4     
   2
dt


1.4 Relative velocity

Let v = speed of each spacecraft as it moves in circle around the centre O.
The relative velocities are denoted by the subscripts A, B and C.
For example, vBA is the velocity of B as observed by A.

The period of circular motion is 1 year T  365  24  60  60 s.                      ………… (28)
                            2
The angular frequency  
                            T
                     L
The speed v              575 m/s                                                    ………… (29)
                 2cos30

                                                                                                        6
                                                        Theoretical Competition:       Solution
                                                                  Question 1         Page 7 of 7


The speed is much less than the speed light  Galilean transformation.

In Cartesian coordinates, the velocities of B and C (as observed by O) are

                           vCB
                                          v
                                              vCA
                                                                   B
                                                    L
                                                                            vBA
                           C
   ĵ
                                                    O
                                                                        v
                                                                  vBC
               î                     L
                                                            L

                                  vAB

                                  v                     A

                                  vAC



For B, vB  v cos 60ˆi  v sin 60ˆj

For C, vC  v cos 60ˆi  v sin 60ˆj

Hence vBC  2v sin 60ˆj   3vˆj
The speed of B as observed by C is 3v  996 m/s                                    ………… (30)

Notice that the relative velocities for each pair are anti-parallel.

Alternative solution for 1.4

One can obtain vBC by considering the rotation about the axis at one of the spacecrafts.

                      2
vBC   L                         (5 106 km)  996 m/s
              365  24  60  60 s


                                                                                              7
