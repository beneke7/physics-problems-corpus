---
id: solution-document-ipho-2011-e1-s
source: ipho
kind: official_solution_document
language: en
solution_type: official
source_pdf: cache/phoxiv/ipho/2011/E1_S.pdf
extraction_method: pdftotext-layout
mapped_problems: [ipho-2011-e1]
verification_status: pending
provenance_note: "Source-provided solution document; preserved from cache/phoxiv/ipho/2011/E1_S.pdf."
---

Experimental Competition:                        14 July 2011
                                                                  Question 1                           Page 1 of 7

Part 1. Calibration

From the relationship between f and C given,
                                                                    1      1         CS
                            f                                                  C
                                        C         CS                f
                                  1
That is, theoretically, the graph of on the Y-axis versus C on the X-axis should be linear of
                                  f
                                       1      CS
which the slope and the Y-intercept is    and     respectively.
The table below shows the measured values of C (plotted on the X-axis,) f and,
                1
additionally,     , which is plotted on the Y-axis.
                f


C (pF) f (kHz) 1/f (ms)
    33 13.94 0.0717                                             1/f vs. C graph
    68 8.30 0.1205                              0.4000
    82 6.99 0.1431                              0.3500
   151 4.17 0.2398                                                                   y = 0.0014x + 0.0251
                                                0.3000
   233 2.79 0.3584

                                  1/f (1/kHz)
                                                0.2500
   219 2.98 0.3356
                                                0.2000
   184 3.48 0.2874
                                                0.1500
   150 4.20 0.2381
                                                0.1000
   115 5.24 0.1908
                                                0.0500
   101 5.89 0.1698
                                                0.0000
                                                         0    50               100             150          200       250
                                                                                      C (pF)

                                  1                                 CS
From this graph, the slope (              ) and the Y-intercept (        ) is equal to 0.0014 s/nF and 0.0251
ms respectively.

                          1         1
Hence,              =        =              = 714 nF/s
                        slope 0.0014 s / nF
                        Y       intercept     0.0251 ms
and              CS =                     =               = 17.9 pF                    as required.
                                slope       0.0014 s / nF




                                                                                                                  1
                                  Experimental Competition:            14 July 2011
                                                Question 1                 Page 2 of 7


Part II. Determination of geometrical shape of parallel-plates capacitor

PATTERN I: The expected graph of C versus the position
               C




                         w       2w      3w      4w        5w   Distance



PATTERN II: The expected graph of C versus the position

               C




                         w       2w      3w      4w        5w   Distance

PATTERN III: The expected graph of C versus the position

               C




                         w       2w      3w      4w        5w   Distance




                                                                                    2
                                     Experimental Competition:           14 July 2011
                                                   Question 1                  Page 3 of 7

 By measuring f andC versus x (the distance moved between the two plates,) the data and
the graphs are shown below.

              x (mm)     f (kHz)     C (pF)   x (mm)  f (kHz)       C (pF)
                     0        7.41       77.9      30      4.94        126.1
                     1        8.09       69.8      31      5.52        110.9
                     2        8.64       64.2      32      6.19         96.9
                     3        9.30       58.3      33      6.48         91.7
                     4        9.30       58.3      34      6.64         89.1
                     5        8.21       68.5      35      5.72        106.4
                     6        7.02       83.3      36      5.08        122.1
                     7        6.40       93.1      37      4.39        144.2
                     8        5.98      100.9      38      4.06        157.4
                     9        5.91      102.4      39      3.97        161.4
                    10        6.38       93.5      40      4.32        146.8
                    11        6.96       84.1      41      4.86        128.5
                    12        7.61       75.4      42      5.33        115.5
                    13        8.40       66.5      43      6.05         99.6
                    14        8.20       68.6      44      5.98        100.9
                    15        7.13       81.7      45      5.14        120.5
                    16        6.37       93.6      46      4.47        141.3
                    17        5.96      101.3      47      3.93        163.3
                    18        5.38      114.3      48      3.74        172.5
                    19        5.33      115.5      49      3.64        177.7
                    20        5.72      106.4      50      3.93        163.3
                    21        6.34       94.2      51      4.30        147.6
                    22        6.85       85.8      52      4.91        127.0
                    23        7.53       76.4      53      5.46        112.3
                    24        7.23       80.3      54      5.49        111.6
                    25        6.33       94.3      55      4.64        135.4
                    26        5.56      110.0      56      4.07        157.0
                    27        5.36      114.8      57      3.62        178.8
                    28        4.73      132.5      58      3.36        194.1
                    29        4.53      139.2




                                                                                          3
                                 Experimental Competition:     14 July 2011
                                               Question 1        Page 4 of 7


        f (kHz)                      f vs. x graph
10.00
 9.00
 8.00
 7.00
 6.00
 5.00
 4.00
 3.00
 2.00
 1.00                                                                 x (mm)
 0.00
           0               10   20        30         40   50     60            70




                  C (pF)
                                     C vs. x graph
250.0




200.0




150.0




100.0




 50.0


                                                                       x (mm)

  0.0
           0               10   20        30         40   50     60            70




                                                                               4
                                         Experimental Competition:                  14 July 2011
                                                       Question 1                      Page 5 of 7



From periodicity of the graph, period = 1.0 cm

Simple possible configuration is:




      b                                                   0.5 cm




                                         1.0 cm

The peaks of C values obtained from the C vs. x graph are provided in the table below.
These maximum C are plotted (on the Y-axis) vs. nodes (on the X-axis.)

node              C_max

                                             C_max (pF)
                                                               C_max vs. Node graph
              1       105.1
                                   200
              2       118.6
              3       139.5        180
              4       163.7        160
                                                                                          y = 19.924x + 82.04
              5       182.1        140
                                   120
                                   100
                                    80
                                    60
                                    40
                                    20
                                                                                                            Node
                                     0
                                         0                 1        2       3         4             5              6



This graph is linear of which the slope is the dropped off capacitance          C   19.9 pF/section.
Given that the distance between the plates d               0.20 mm, K   1.5 ,
                      K 0A
              C               ,
                        d
and       A          5 10 3 m     b mm          10 3 m2



                                                                                                        5
                                     Experimental Competition:                 14 July 2011
                                                   Question 1                     Page 6 of 7

                             C d
Then, b mm                                       60 mm      if medium between plates is the
                    K 0 10 3       5 10 3
dielectric of which K  1.5 .



Part III. Resolution of digital micrometer

From the given relationship between f and C, f                  ,
                                                   C       CS
                                    df
                               f             C                      C
                                    dC           (C    C S )2
                                                 f2
                                                       C

                                             C         f
                                                  f2

And since C linearly depends on x, C     mx                         C   m x.
Hence,

                                         x             f,
                                               mf 2
where f is the smallest change of the frequency f which can be detected by the multimeter,
x 0 is the operated distance at f = 5 kHz, and m is the gradient of the C vs. x graph at
x   x0 .

From the f vs. x graph, at f = 5 kHz, The gradient is then measured on the C vs. x graph
around this range.




                                                                                              6
                                     Experimental Competition:                 14 July 2011
                                                   Question 1                    Page 7 of 7


                                     C vs. x graph 2
            250.0



            200.0

                                        y = 17.455x - 504.54
            150.0

   C (pF)
            100.0



             50.0



              0.0
                    0   10         20             30            40   50          60        70
                                                       x (mm)




From this graph, m      17.5 pF / mm    1.75 10 8 F / m .
Using this value of m, f 5 kHz ,     714 nF/s , and f 0.01 kHz ,
                             714 10 9
                 x                8        3 2
                                                 (0.01 103 ) 0.016 mm
                      (1.75 10 )(5 10 )
NB. The C vs. x graph is used since C (but not f) is linearly related to x .


Alternative method for finding the resolution
(not strictly correct)

Using the f vs. x graph and the data in the table around f 5 kHz , it is found that when f is
changed by 1 kHz ( f 1 kHz ,) x is roughly changed by 1.5 mm ( x 1.5 mm .) Hence,
when f is changed by f      0.01 kHz (the smallest detectable of the change,) the distance
moved is x 0.015 mm .




                                                                                           7
