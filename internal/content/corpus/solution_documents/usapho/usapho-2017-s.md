...
Considering the horizontal and vertical forces on the block gives
                          ...
                          N cos θ + f sin θ = mg,        N sin θ − f cos θ = ma.
...
However, to get full credit, students must account for two edge cases. First, when µ > tan θ, no
force is required at all to keep the block in place, so the minimum force is zero. Second, when
µ > cot θ, the block will not slip up under any circumstances, so there is no maximal force.
...
and taking the tangent of both sides gives
                                         amin    tan θ − µ
                                              =             .
                                          g     1 + µ tan θ
This is only meaningful for tan θ > µ, otherwise the answer is simply amin = 0. At the maximum
acceleration amax , β = θ + φ, which gives
                                         amax    tan θ + µ
                                              =             .
                                          g     1 − µ tan θ
This is only meaningful for cot θ > µ, otherwise the answer is simply amax = ∞.
...
(bow) and directly right of the forward direction (starboard), respectively.
  ...
  a. Derive an expression for the deviation of the compass, δθ, from north as a function of Kb ,
     ...
     We add the fields to get the local field. The northward component is
                            ...
                            Bnorth = Be − Be Kb cos θ cos θ − Be Ks sin θ sin θ

     while the eastward component is

                               Beast = −Be Kb sin θ cos θ + Be Ks cos θ sin θ

     The deviation is given by
                                                           sin θ cos θ
                              tan δθ = (Ks − Kb )                             .
                                                    1 − Kb cos2 θ − Ks sin2 θ
...
2017 USA Physics Olympiad Exam                 Part A                                              9
     ...
     green and black sides is φ, so the length of the red side is a sin φ and the length of the green
     side is a cos φ.
     ...
     where x = a cos φ is the length of the green side, so
                                                      mKi
                                           Br = 2Be       cos φ.
                                                       d3
...
2017 USA Physics Olympiad Exam                    Part A                                           12
     ...
     Squaring both expressions and dropping Eb2 terms, since Eb is small,

                                  p2p − 2pp pb ≈ p2∆ ,    Ep2 + 2Ep Eb ≈ E∆
                                                                          2
                                                                            .
...
2017 USA Physics Olympiad Exam                Part A                                      13
   ...
   The following relationships may be useful in solving this problem:
                             ...
                             relativistic energy          E = γmc2
                                                                 q
                                                          f          1−β
                             relativistic doppler shift   f0 =       1+β
...
and mass m. The domino is free to rotate about its edges, but will not slide across the table.
  ...
  a. Suppose we give the domino a sharp, horizontal impulsive push with total momentum p.
     ...
     height is half the diagonal
                         √        of the domino, so the distance from the contact point to the center
                       1   2     2
     ...
     moment of inertia of the domino about its contact point with the table is I = 13 m(h2 + t2 ).
...
2017 USA Physics Olympiad Exam                Part B                                               16
    ...
    You can find this with an integral, or if you know the moment of inertia about the center
      1
    ( 12 m(h2 + t2 )), you can use the parallel axis theorem. We know the momentum L = ph, so
  ...
  b. Next, imagine a long row of dominoes with equal spacing l between the nearest sides of any
     ...
     total kinetic energy is lost in the collision of the first domino with the second domino?
                                              ...
                                              Solution
                                                             √
    Right after the collision, the dominoes touch at a height h2 − l2 above the table. The second
    ...
    that are in contact, they must have the same horizontal velocity component in order  √ for the
                                                                                            2    2
    dominoes to stay in contact. For the first domino, this velocity is ω1 h sin
                                                                               √  θ = ω 1 h −l .
    For the second domino it is ω2 times the height of the impact point, so ω2 h2 − l2 . Because
...
2017 USA Physics Olympiad Exam               Part B                                               17
  ...
  c. After the collision, the dominoes rotate in such a way so that they always remain in contact.
    ...
    horizontally a distance h sin(φ + α) which we will approximate as h(φ + α) using the small
    angle approximation. The top left corner of the second domino moves horizontally forward
    by h sin(φ + β) + t(1 − cos(φ + β)). We ignore the cosine term as second order in the rotation
    angle and approximate this as h(φ + β).
    The y-coordinate of the upper right corner of the first domino is h(1 − cos(φ + α)) ≈ h. The
    y-coordinate of the upper left corner of the second domino is h(1 − cos(φ + β)) + t sin(φ + β) ≈
