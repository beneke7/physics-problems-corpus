---
id: kevin-zhou/Prelim
source: Kevin Zhou Physics Olympiad Handouts
kind: handout
language: en
translated: false
bidirectional_links:
  hu: corpus/notes_hu/kevin-zhou/Prelim.md
source_file: sources/kevin_zhou/text/handouts/Prelim.txt
source_url: https://knzhou.github.io/handouts/Prelim.pdf
source_commit: ebb627ccb7e27e541d64011f22d36766a6e97ecf
verification_status: pending
includes_solution_pdf: false
provenance_note: Complete extracted English Kevin Zhou Physics Olympiad Handouts Prelim; PDF headers and footers removed where present; solution PDFs excluded from this corpus note.
---
...
### [1] Problem 1. Projectile Pass Through Points

A projectile is thrown up and passes point A, then point B a height $h$ above A. Let $T_A$ and $T_B$ be the time intervals between the two times the projectile passes A and B, respectively.

**(a)** Show that an experimentalist can measure $g$ by computing $g = \frac{8h}{T_A^2 - T_B^2}$.

**(b)** This procedure probably looks a little contrived. Why is it better than doing something simpler, such as just dropping the ball and using $\Delta y = \frac{gt^2}{2}$?

### [2] Problem 2. Projectile Range and Height

A projectile is launched with speed $v$ at an angle $\theta$ from the horizontal on a flat plane.
...
### [1] Problem 3. Pulley Spring Scale System

Consider the two following setups involving pulleys and spring scales. Treat the ropes and spring scales as massless, and the pulleys as frictionless. Model the pulleys as uniform discs with masses of 5 kg which are fixed by a rigid support.
...
**(d)** For $g = 10\ \mathrm{m/s^2}$, what is the magnitude of the force that must be provided by the support?

### [1] Problem 4. Ladder in Circular Aqueduct

A sewage worker is using a ladder inside a large, frictionless, horizontal circular aqueduct. The ladder is of the same length as the diameter of the aqueduct.
...
### [1] Problem 5. Fool's Tackle Pulley System

A student proposes to build this set of pulleys and string, called a "fool's tackle".
...
### [2] Problem 6. Triangle Tipping and Sliding

A wooden isosceles right triangle with uniform mass density is placed on a table, and a force is applied as shown.
...
### [1] Problem 7. Painter on Rope-Suspended Platform

A painter of mass $M$ stands on a platform of mass $m$ as shown.

He pulls each rope down with force $F$, and accelerates upward with acceleration $a$. Find $a$.

### [1] Problem 8. Rolling Wheel Rim Speed

A wheel of radius $R$ is rolling without slipping with angular velocity $\omega$. What is the average speed $v$ of a point on the rim? How about the average of $v^2$?

### [3] Problem 9. Spherical Bowl Oscillations

A small block lies at the bottom of a spherical bowl of radius $R$.
...
### [2] Problem 10. Accelerating Car Door

A car accelerates forward uniformly from rest. Initially, its door is very slightly ajar. Calculate how far the car travels before the door slams shut. Assume the door has a frictionless hinge at its front, a uniform mass distribution, and a length $L$ from front to back.

### [2] Problem 11. Ant Walking on Ring
...
### [2] Problem 12. Baseball Bat Sweet Spot

A baseball player holds a bat, modeled as a uniform rigid rod, horizontally at one of its ends. Usually, when the baseball hits the bat, the player will feel a sharp jolt in their hands as the bat recoils. This can be avoided if the baseball hits the "sweet spot". Where is it?

### [3] Problem 13. Maxwell's Wheel

Maxwell's wheel is a toy which demonstrates conservation of energy. It consists of a uniform disc of mass $M$ and radius $R$, with a massless axle of radius $r \ll R$.
...
### [3] Problem 14. Cue Ball Billiards

A cue ball is a uniform sphere of radius $R$.
...
### [4] Problem 15. Hexagon Collision

Six identical uniform rods, fastened at their ends by frictionless massless pivots, form a regular hexagon and lie on a frictionless surface.
...
### [2] Problem 16. Satellite Elliptical Orbits

Several possible elliptical orbits of a satellite are shown below.
...
### [2] Problem 17. Comet Parabolic Path

A comet passes by the Sun as shown, in a parabolic path.
...
### [2] Problem 18. Plumb Bob Deviation

Because of the rotation of the Earth, the line of a plumb bob will not align with the local gravitational field. Find the (small) angle of deviation between them as a function of the latitude $\theta$, the gravitational acceleration $g$, the radius $R$ of the Earth, and its angular velocity $\omega$.

### [2] Problem 19. Cylindrical Shell Integration

You should be comfortable with setting up multiple integrals. Consider a cylindrical shell whose axis of symmetry is the z-axis. It has non-uniform mass per unit area $\sigma(\phi, z)$ in cylindrical coordinates, and the shell has radius $a$ and height $h$, with the bottom edge at $z = 0$.
...
### [2] Problem 20. Balloon Propulsion Paradox
...
### [2] Problem 21. CSEM Test

As a warmup, take the Conceptual Survey of Electricity and Magnetism test. This should take no longer than 1 hour. You do not have to justify your answers; just list them.

### [2] Problem 22. Half-Infinite Line Charge

A half-infinite line has linear charge density $\lambda$.
...
### [2] Problem 23. Vector Field Visualization

Some basic tasks involving intuition for vector fields.
...
### [3] Problem 24. Charged Parallel Plate Capacitor

A parallel plate capacitor of capacitance $C$ is placed in a region of zero electric field. The first plate is given total charge $Q_1$ and the second plate is given total charge $Q_2$. Let the plates have area $A$ and separation $d$, where $A \gg d^2$.
...
**(d)** In addition to the force between the plates found in part (c), there is a contribution to the internal stress (force per unit area) within each plate due to the charges on its two surfaces. Find this part of the stress for each plate, and assuming $Q_1 > Q_2 > 0$, indicate whether it is tension or compression.

### [3] Problem 25. RC Circuit Transient

A battery is connected to an RC circuit as shown.
...
**(d)** Suppose that the emf $E(t)$ supplied by the battery can be adjusted freely over time, and the capacitor must be given a total charge $Q$ by the time $t = T$. Sketch the profile $E(t)$ that maximizes the efficiency of this process, i.e. the ratio of the energy stored in the capacitor to the energy output by the battery, and find this efficiency.

### [2] Problem 26. Neuron Firing Speed

In this problem we estimate the maximum firing speed of a human neuron. Model a human cell simply as a sphere of radius $10^{-6}\ \mathrm{m}$.

**(a)** It has been measured that 1 cm² of cell membrane has a resistance of 1000 Ω. Estimate the resistance of a single human cell.
...
### [2] Problem 27. Solenoid Induced Ring Current

An infinite solenoid with radius $b$ has $n$ turns per unit length. The current varies in time according to $I(t) = I_0 \cos \omega t$. A ring with radius $r < b$ and resistance $R$ is centered on the solenoid's axis, with its plane perpendicular to the axis.
...
### [2] Problem 28. Rotating Charged Cylinder

A very long, insulating cylinder with radius $r$ and uniform surface charge density $\sigma$ on its outer surface rotates about its symmetry axis with angular velocity $\omega$.
...
### [2] Problem 29. Rectangular Loop Momentum

A rectangular loop of wire with dimensions $a$ and $b$ is placed with one side parallel to a long, straight wire carrying current $I_0$, at a distance $l$.
...
### [1] Problem 30. AC Capacitor Light Bulb

A 120 V rms, 60 Hz line provides power to a 40 W light bulb, modeled as a resistor. How will the brightness change if a 10 µF capacitor is connected in series with the light bulb?

## 3 Thermodynamics

These problems can be solved using the material in chapters 21 through 24 of Halliday and Resnick.

### [2] Problem 31. Thermodynamic Cycle

Two moles of a monatomic ideal gas are taken through the following cycle.

- The gas begins at point A with pressure $P_0$ and volume $V_0$.
...
### [3] Problem 32. Thermodynamic Derivations

Deriving some basic results in thermodynamics.

**(a)** Starting from the first law of thermodynamics, derive the fact that $PV^\gamma$ is constant for an ideal gas in an adiabatic process.

**(b)** Using the ideal gas law, derive the total work done by a gas as it expands at constant temperature from volume $V_1$ to $V_2$, in terms of $n$, $R$, $T$, $V_1$, and $V_2$.

**(c)** Show that if a general gas, not necessarily ideal, satisfies the equation $PV = kU$, where $U$ is the total internal energy, then $PV^n$ is constant in an adiabatic process for some power $n$, and find $n$ in terms of $k$.
...
### [1] Problem 33. Adiabatic Compression

A monatomic ideal gas is adiabatically compressed to 1/8 of its original volume. For each of the following quantities, indicate by what factor they change.

**(a)** The rms velocity $v_\mathrm{rms}$.

**(b)** The mean free path $\lambda$.

**(c)** The average time between collisions $\tau$ for each gas molecule.

**(d)** The molar heat capacity $C_v$.

### [2] Problem 34. Heat Engine Cycle Efficiency

A simple heat engine consists of a movable piston in a cylinder filled with an ideal monatomic gas. Initially the gas in the cylinder is at a pressure $P_0$ and volume $V_0$. The gas is slowly heated at constant volume until the pressure is $32P_0$. The gas is then adiabatically expanded until its pressure is $P_0$ again. Finally, the gas is cooled at constant pressure until its volume is $V_0$ again. Find the efficiency of the cycle.

### [2] Problem 35. Hot-Air Balloon Lift

The total mass of a hot-air balloon (envelope, basket, and load) is 320 kg. Initially the air pressure inside and outside the envelope is $1.01 \times 10^5\ \mathrm{Pa}$ and its density is $1.29\ \mathrm{kg/m^3}$. In order to raise the hot-air balloon, a gas burner is used to heat the air inside the balloon. The volume of the envelope filled with hot air is $650\ \mathrm{m^3}$. The molar mass of air is 29 g/mol. Treat the temperature of the air in the balloon as uniform.
...
### [3] Problem 36. Ice in Heating Kettle

Water is heated in an electric kettle. At some time, a piece of ice at temperature $T_0 = 0\ ^\circ\mathrm{C}$ was put in the kettle. The figure below shows the water temperature as a function of time.

Find the mass of the ice if the heating power of the kettle is $P = 1\ \mathrm{kW}$. The latent heat of melting for ice is $L = 335\ \mathrm{kJ/kg}$, the heat capacity of water is $c = 4.2\ \mathrm{kJ/(kg \cdot K)}$, and the temperature of the room is $T_1 = 20\ ^\circ\mathrm{C}$. (Hint: it's very easy to get an answer that's off by up to 50% if you're careless.)
...
### [2] Problem 37. Train Platform Bomb Simultaneity
...
### [1] Problem 38. Photon Emission Recoil

An atom with mass $m$ is at rest, then radiates a photon with angular frequency $\omega$. What is the rest mass of the atom afterward?

### [3] Problem 39. Sagging Rope Standing Waves

A rope of linear mass density $\sigma$ is hung between two poles, a distance $L$ apart, and the middle of the rope sags a distance $d \ll L$ below the ends. Find the approximate frequencies of standing waves on the rope. (Hint: you do not have to solve for the shape of the rope. As an intermediate step, it will be useful to consider torque balance on half of the rope.)

### [3] Problem 40. String with Attached Mass

A uniform string of length $L$ and linear density $\rho$ is stretched between two fixed supports. The tension in the string is $T$.
...
### [2] Problem 41. Glass-Plastic Interference Fringes

A perfectly flat piece of glass is placed over a perfectly flat piece of black plastic.
...
### [2] Problem 42. Lloyd's Mirror

A point source of light L emitting a single wavelength $\lambda$ is situated a distance $d$ from an ideal mirror, at $z = 0$. A screen stands at the end of the mirror at distance $D \gg d$ from L.
...
### [2] Problem 43. Galileo's Argon Atoms

Argon atoms are special because they stay in the atmosphere for a very long time. They are not recycled like oxygen and nitrogen. An average breath inhales around 0.5 L of air and people breathe on average around once every five seconds. Air is about 1% argon and has density $1.2\ \mathrm{kg/m^3}$. Assume all air particles have a mass of approximately $5 \times 10^{-26}\ \mathrm{kg}$. Take the atmosphere to have constant density and be around 20 km thick. The radius of the Earth is $6.4 \times 10^6\ \mathrm{m}$.
...
### [3] Problem 44. Pendulum g Measurement

The acceleration due to gravity can be measured by measuring the time period of a simple pendulum. However, it can be challenging to get an accurate result.
...
### [2] Problem 45. Blackbody Radiation Scaling

Blackbody radiation is an electromagnetic phenomenon, so the radiation intensity depends on the speed of light $c$. It is also a thermal phenomenon, so it depends on the thermal energy $k_B T$, where $T$ is the object's temperature and $k_B$ is Boltzmann's constant. And it is a quantum phenomenon, so it depends on Planck's constant $h$.
