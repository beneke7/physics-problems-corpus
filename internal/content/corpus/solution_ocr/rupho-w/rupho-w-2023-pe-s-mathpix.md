---
id: solution-ocr-rupho-w-2023-pe-s
source: rupho-w
...
---
А1 ${ } ^ { 0.41 }$ Для каждого $x$ найдите $n$.
...
A3 ${ } ^ { 0.82 }$ Для каждого $x$ найдите $\mathcal { J } _ { \lambda }$ (в условных единицах на нм). Поскольку для каждого $x$ мы уже вычислили $\lambda$, мы получили зависимость $\mathcal { J } _ { \lambda } ( \lambda )$.

Ответ: См. А6.

A4 ${ } ^ { 0.49 }$ Постройте график $\mathcal { J } _ { \lambda } ( \lambda )$.

Ответ:
![](../../../figures/solution-ocr/e4a289e0803fffab3c2e5477.jpg)

А6 ${ } ^ { 1.23 }$ Найдите для каждого $x$ значение $\sigma$. Поскольку для каждого $x$ мы нашли $\lambda$, мы получили также зависимость $\lambda ( \sigma )$.

Для каждого $x _ { i }$ найдём значение суммы

$$
\Sigma _ { i } = 2 \sum _ { k = 1 } ^ { i - 1 } \mathcal { J } _ { x _ { k } } + \mathcal { J } _ { x _ { i } } ,
$$

а также нормировочный множитель

$$
\Sigma = 2 \sum _ { i } \mathcal { J } _ { x _ { i } } .
$$

Поскольку $\lambda ( x )$-- убывающая функция, то $\sigma = 0$ соответствует значение $x = x _ { \max }$, поэтому $\sigma _ { i }$ рассчитывается по формуле:

$$
\sigma _ { i } = 1 - \frac { \Sigma _ { i } } { \Sigma } .
$$

Ответ:

| $x$ | $\mathcal { J } _ { x }$ | $n$ | $\lambda$, нм | $\mathcal { J } _ { \lambda }$ | $\sigma$ |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
Действуем аналогично А6.

Ответ: См. В3.

B3 ${ } ^ { 4.10 }$ С помощью линейной интерполяции найдите, каким значениям $\lambda$ соответствуют полученные значения $\sigma$. Поскольку $n$ были вычислены в B1, мы получаем искомую зависимость $n ( \lambda )$.

Ответ:

| $x ^ { \prime }$ | $\mathcal { J } _ { x ^ { \prime } }$ | $n$ | $\sigma$ | $\lambda$ |
| :--- | :--- | :--- | :--- | :--- |
...
Ответ:
![](../../../figures/solution-ocr/a92f7f5cd4672a6c37120f00.jpg)
