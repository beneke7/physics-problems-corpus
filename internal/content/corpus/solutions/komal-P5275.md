---
id: komal-P5275
source: komal
language: hu
translated: false
problem: komal-P5275
figure_files: []
provenance_note: "KöMaL official solution (Hungarian)"
---

**Megoldás.**
 $a)$ 60 másodperc alatt 1 m$^3$ vizet 5 méter magasba emelni
 $P_\text{hasznos}=\frac{1000~{\rm kg}\cdot
9{,}81~{\rm N/kg} \cdot 5~{\rm m}}{60~\rm s}=0{,}82~\rm kW$
 teljesítménnyel lehet. A villanymotor felvett teljesítménye tehát
 $P_\text{motor}=\frac{P_\text{hasznos}}{\eta}=
\frac{0{,}82~{\rm kW}} {0{,}75}= 1{,}1~{\rm kW}.$
 $b)$ A vízsugár akkor emelkedik 5 méter magasra, ha a kezdősebessége
 $v_0=\sqrt{2gh}=\sqrt{2\cdot 5~{\rm m}\cdot 9{,}81~ {\rm
m/s}^2}=9{,}9~\frac{\rm m}{\rm s}.$
 $c)$ Az $A$ nagyságú keresztmetszeten $v_0$ sebességgel kiáramló víz térfogata másodpercenként $Av_0$:
 $\frac{1~\rm m^3}{60~\rm s}=A\cdot 9{,}9~\frac{\rm
m}{\rm s},$
 ahonnan
 $A=\frac{d_0^2\pi}{4}=16{,}8~\rm cm^2,$
 vagyis a vízsugár (kezdeti) átmérője
 $d_0=2\sqrt{\frac{A}{\pi}}=4{,}6~\rm cm.$
 $d)$ 2,5 méter magasságban egy adott tömegű vízdarabka mozgási energiája csak a fele a kezdeti értéknek, a víz áramlási sebessége tehát
 $v_1=\sqrt{\frac{1}{2}}v_0=7{,}0~\frac{\rm m}{\rm s}.$
 Az anyagmegmaradás törvénye szerint amilyen arányban csökken a vízsugár sebessége, ugyanolyan arányban növekszik a keresztmetszete. Másrészt a vízsugár átmérője a keresztmetszet négyzetgyökével arányos, így a kérdezett átmérő
 $d_1= \sqrt[4]{2}\cdot 4{,}6~\rm cm \approx 5{,}5~\rm cm.$

 Megjegyzés. A megfontolásaink során elhanyagoltuk az áramló ,,vízdarabkák'' sebességének vízszintes komponensét, pedig ilyen biztosan van, mert a lelassuló vízsugár fokozatosan kiszélesedik. Ugyancsak eltekintettünk a vízsugár (előbb vagy utóbb bekövetkező) cseppekre szakadásától, és nem törődtünk a felülről lefelé visszahulló vízcseppekkel sem. A vízsugár alsó részénél ez jó közelítés lehet, de a tetejénél (ahol a függőleges sebességkomponens nullává válik) biztosan nem alkalmazható.
