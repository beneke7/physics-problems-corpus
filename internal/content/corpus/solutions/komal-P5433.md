---
id: komal-P5433
source: komal
language: hu
translated: false
problem: komal-P5433
figure_files: [komal-P5433-sol-fig1.gif]
provenance_note: "KöMaL official solution (Hungarian)"
---

**Megoldás.**
 A fény $\alpha$ beesési szög esetén
 $\beta=\arcsin\frac{\sin\alpha}{n}$
 szögtben halad tovább a vízben. Mivel $\sin\alpha<1,$
 $\beta<\arcsin\frac34=48{,}6^\circ.$
 A törés után a fénysugár többször tükröződhet az akvárium hosszabb oldalfalain és egyszer a rövidebben. Ha tükrözzük az akvárium felülnézeti ábráját az oldalfalakra és a hátsó falára, a fénysugár útja egyenessé válik (lásd az ábrát) .

 Tegyük fel, hogy mire a fénysugár visszaérkezik az akvárium tükrözésmentes falához (az ábrán a bal szélső egyeneshez), a hosszabb oldalakon $k$-szor verődik vissza. Amennyiben $k$ páratlan szám, a másodszor is megtörő fénysugár az eredetivel párhuzamosan halad a levegőben. Ez a feltétel akkor teljesül, ha a fény az ábrán halványszürkén jelölt sávok valamelyikében éri el a bal szélső vonalat. A geometriai viszonyokból adódik, hogy
 $(k-1)d< 2L\tan\beta<2L\cdot \tan48{,}6^\circ=272{,}2\ \rm cm,$
 vagyis
 $k< 6{,}4.$
 Figyelembe véve, hogy $k$ páratlan, $k_\text{max}=5,$ vagyis (a rövidebb oldalon történő egyetlen visszaverődéssel együtt) a fénysugár legfeljebb 6-szor verődhet vissza.

![solution figure](../../figures/komal/figures/komal-P5433-sol-fig1.gif)
