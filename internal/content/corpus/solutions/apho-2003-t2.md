---
id: apho-2003-t2
source: apho
language: en
solution_language: en
translated: false
problem: apho-2003-t2
solution_type: official
source_document: solution-document-apho-2003-t2-s
source_pdf: cache/phoxiv/apho/2003/T2_S.pdf
figure_files: []
provenance_note: "Source-provided solution section from cache/phoxiv/apho/2003/T2_S.pdf."
---

Solution and Marking Scheme
Theory
II. Optical Gyroscope
The light wave moves with speed
µ
c
c = ′ in the medium having refractive
index µ . Wavelength of light in medium
µ
λ
λ = ′ , where λ is the wavelength of light
in vacuum.
a) (2 points)
transit time for the CW beam: 1
) 1 (
2 2 −
+
+
′
Ω
−
′
=
′
Ω +
=
c
R
c
R
c
t R R
t
π π
transit time for the CCW beam: 1
) 1 (
2 2 −
−
−
′
Ω
+
′
=
′
Ω −
=
c
R
c
R
c
t R R
t
π π
the time difference between +
t and −
t :
( ) 2 2 2
2
4
Ω − ′
Ω
= ∆
R c
R
t
π
since ( ) ( )2 2
c R ′ << Ω
( )2
2
4
c
R
t
′
Ω
≈ ∆
π
b) (2 points) the round-trip optical path difference, L ∆ , is given by
c
R
t c L
′
Ω
= ∆ ′ = ∆
2
4π
c) (1 point) ∆L ≅ 4.5x10-12
m.
d) (1 point) the corresponding optical phase difference θ ∆ is,
'
8
'
2 2 2
λ
π
λ
π
θ
c
R L Ω
=
∆
= ∆ , where
µ
λ
λ = '
for N turns of fiber optic ring,
'
8 2 2
λ
π
θ
c
N R Ω
= ∆
e) (2 points)
The figure shows the triangular ring rotating about the centre o with the angular speed
Ω in the clockwise direction. Without loosing generality, let‘s first consider the
velocity of light along AC in the CW and CCW direction,
h c R c v Ω ± = Ω ± = ± θ cos , where h is constant.
) 1 (
3 / 3 / 3 /
c
h
c
L
h c
L
v
L Ω
≈
Ω ±
= =
±
± ∓ τ
where τ± is the time taken for light travelling along AC in the CW and CCW.
) 1 (
c
h
c
L
h c
L
v
L
t
Ω
≈
Ω ±
= =
±
± ∓ , where L is the perimeter of the triangular
ring.
Therefore, the time difference of light travelling in one complete cycle.
2 2 2
4
2
1 4 2
c
A
Lh
c c
Lh
t
Ω
= 




 Ω
=
Ω
= ∆ , where A is the area of the triangular ring.
f) The resonance frequencies associated with ± L corresponding to the effective cavity
lengths seen by CW and CCW propagating beams respectively is,
) 1 (
) 1 (
c
h
L ct L
c
h
L ct L
Ω
+ ≈ =
Ω
− ≈ =
−
−
+
+
where L± is the perimeter of the equilateral triangle in the CW (+) and CCW (-) and
we also use the fact that hΩ << c. Therefore,
2
4
2
3
h A L
L L L L
c c c
− +
Ω Ω Ω
∆ = − = = =
The condition to sustain the laser oscillation (given in the problem),
c
L
m
±
± = ν , m = 1, 2, 3, … integers (1 point)
L
L
L
L
mc c
L
m
c
L
m ∆
=
∆
≈ − = − = ∆
+ −
+ − ν ν ν ν 2
(1 point)
θ
θ
h
R RΩ
L/3
O
A
B C
RΩ cosθ
the approximation arises from 2
L L L ≈ − +
where L is the perimeter of the triangular ring. Hence,
Ω = Ω =
∆
= ∆
λ
ν ν ν
L
Lc
A
L
L
3
1 4
(1 point)
