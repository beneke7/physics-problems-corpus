...
source_document: solution-document-ipho-2016-t1-s
source_pdf: cache/phoxiv/ipho/2016/T1_S.pdf
figure_files: []
provenance_note: "Source-provided solution section from cache/phoxiv/ipho/2016/T1_S.pdf."
---
...
0.2
2
In terms of d rather than b gives 0.1pts rather than 0.2pts for the
nal answer: 0.1
IS =
1
2
...
oor. You may assume that the height H of the tower is small,
...
tower) has rotated when the mass lands on the
oor, see
...
ωss(R − H)cosφ 0.1
(Student may also write cosα instead of cosφ, since dx
 H.)
cosφ =
R − H
R
...
v = ωss(R − H) 0.1
distance d that the mass
ies before hitting the space station
d2
= R2
− (R − H)2
0.1
...
φ = tanφ 0.2
Realizing that there is an in
nite number of solutions. 0.2
...
. The goal is to
nd an approximate
solution for the second intersection. The
rst intersection is discarded   it is introduced
...
p
1 − x2/x) =: g(x)
• g(x) is then smaller than x up to the
rst solution. In particular it is negative in
some region (see
gure 4). Finding the third zero thus gives a lower bound for the
solution: √
1 − x2
x
= 5π/2
...
If the student plots f rather than g,
nd solution to f = 1: is equivalent to the
...
x = 0, y = 0, and then lets it go (see
gure 4).
10
• Give an algebraic expression of x(t) and y(t). You may assume that ωssd is small.
